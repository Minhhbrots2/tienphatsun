					...
					<div class="card no-shadow">
						<div class="card-body">
							<ul id="myUL" class="fs-14">
								<li>
									<div class="caret fw-bold fs-20 text-main">{$_oProject.project_name}</div>
												...
												<li>
													...
													<ul class="ul_parent project nested active">
														{foreach from=$menu_child_project key=key_menu_child item=val_menu_child}
															<li class="nested_child">{$key_menu_child} {if !empty($val_menu_child)}<i class='bx bx-check fs-24 fw-bold text-success'></i>{else}<i class='bx bx-x-circle fw-bold fs-24 text-danger' ></i>{/if}</li>
														{/foreach}
													</ul>
												</li> 
											{else}
												<li class="nested_child"><span class="text-dark fs-16">{$_oMenuProject.cat_name}</span> {if !empty($_oMenuProject.status)}<i class='bx bx-check fs-24 fw-bold text-success'></i>{else}<i class='bx bx-x-circle fw-bold fs-24 text-danger' ></i>{/if}</li>
											{/if}
										{/foreach}
										{if !empty($list_block)}
												...
												<li>
													...
													<ul class="ul_block ul_parent nested active">
														{foreach from=$menuBlock item=_oMenuBlock}
															...
															{else}
																<li class="nested_child"><span class="text-dark fs-16">{$_oMenuBlock.cat_name}</span> {if !empty($_oMenuBlock.status)}<i class='bx bx-check fs-24 fw-bold text-success'></i>{else}<i class='bx bx-x-circle fw-bold fs-24 text-danger' ></i>{/if}</li>
															{/if}
														{/foreach}
																...
																<li>
																	...
																	<ul class="ul_parent nested active">
																		{foreach from=$menuBuilding item=_oMenuBuilding}
																			...
																			{else}
																				<li class="nested_child"><span class="text-dark fs-16">{$_oMenuBuilding.cat_name}</span> {if !empty($_oMenuBuilding.status)}<i class='bx bx-check fs-24 fw-bold text-success'></i>{else}<i class='bx bx-x-circle fw-bold fs-24 text-danger' ></i>{/if}</li>
																			{/if}
																		{/foreach}
																	</ul>
																</li>
