...
<div class="container-xxl flex-grow-1 pt-2 container-p-y" >
	<div class="d-flex align-items-center mb-2 flex-wrap gap-2 justify-content-between">
		<div class="p__left mb-lg-0">
			<h4 class="fw-bold mb-1 {if $deviceType eq 'phone'}fs-16{/if}">Danh sách cập nhật cao tầng đại lý</h4>
		...
		<div class="p__right d-flex gap-1 {if $deviceType eq 'phone'}justify-content-end flex-fill{/if}">
			...
			<button class="btn btn-default btn-icon " type="button" onclick="$Core.crawl.open_config_update_stock(this,event)" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}">
						...
						<tr>
							{foreach from=$list_blocks item = _block_name key=key name=i}
								<th class="align-center h-px-40 text-center no-sticky" width="20%" {if $smarty.foreach.i.last}style="border-right: 1px solid #d9dee3"{/if}>
									<div class="d-flex align-items-center justify-content-center gap-1">
										{$_block_name}
										<span class="fw-bold fs-20 text-warning">{$arr_total_block[$key]}</span>
									</div>
								</th>
							{/foreach}
						</tr>
							...
							{foreach from=$list_agency item=_oItem name=i }
								...
								<tr class="tr_agency tr_agency_{$_oItem.property_id}" >
									...
									{foreach from=$list_blocks item=_block_name key=key}
										<td class="text-center {if $block_crawl[$key].is_success eq '0'}not_success{elseif !empty($block_crawl[$key].is_crawl) && !empty($block_crawl[$key].is_changed)}is_changed{/if}" {if $block_crawl[$key].is_success eq '0'}style="background:#ffe1e1"{elseif !empty($block_crawl[$key].is_crawl) && !empty($block_crawl[$key].is_changed)}style="background:#ffff9c !important"{/if}>
											<div class="d-flex align-items-center justify-content-start gap-1 flex-wrap">
												{if !empty($block_crawl[$key].is_crawl)}
													<div class="d-flex gap-1 align-items-center justify-content-between">
														<div class="fs-11 d-flex flex-wrap gap-1 align-items-center lst_action_crawl">
															<div class="d-flex align-items-center gap-1">
																<a class="fs-5" href="javascript:void(0);" onclick="$Core.crawl.open_import_logs(this, event)" agency_id="{$_oItem.property_id}" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}" target_id="{$key}" data-toggle="tooltip" title="" data-original-title="Lịch sử cập nhật"><i class="fa fa-history" aria-hidden="true"></i></a>
																<button agency_id="{$_oItem.property_id}" type="button" class="btn btn-outline-primary btn-sm text-nowrap btn_crawl" onClick="{if $clsISO->_DEV()}$Core.crawl.crawl_agency1(this,event){else}$Core.crawl.crawl_agency(this,event){/if}" project_id="" block_id="{$key}" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}">
																	<i class="fa fa-play"></i> 
																	<i class="fa loading d-none fa-circle-o-notch fa-spin fa-fw"></i> 
																	<span>Thực hiện</span>
																</button>
																<button agency_id="{$_oItem.property_id}" type="button" class="btn btn-success btn-sm text-nowrap" onClick="$Core.crawl.start_import(this,event)" target_id="{$key}" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}" spreadsheetId="{$more_information.spreadsheetId}">
																	<i class="fa fa-play"></i> 
																	<i class="fa loading d-none fa-circle-o-notch fa-spin fa-fw"></i> 
																	<span>Crawl</span>
																</button>
																<form action="" enctype="multipart/form-data">
																	<button onclick="$Core.crawl.choose_image(this,event)" agency_id="{$_oItem.property_id}" class="btn btn-sm btn-danger text-nowrap"><i class="fa fa-upload"></i> <span>Image</span></button>
																	<input type="file" name="images[]" onchange="$Core.crawl.start_import_image(this,event)" data-type="_IMAGE" agency_id="{$_oItem.property_id}" target_id="{$key}" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}" class="d-none file_upload" multiple="">
																</form>
															</div>
															<div class="d-flex gap-1 align-items-center flex-wrap gap-1">
																<div class="text-nowrap"><a href="{$clsISO->genGoogleURL($block_crawl[$key].sheetID,'spreadsheets')}" target="_blank" class="limit_1line">Bảng hàng nguồn <i class='bx bx-link-external fs-11'></i></a></div>
																<div class="text-nowrap"><a href="{$block_crawl[$key].link_stock}" target="_blank" class="limit_1line text-main">Bảng hàng <i class='bx bx-link-external fs-11'></i></a></div>
																<div class="text-nowrap text-left w-100">Cập nhật: <strong class="text-main">{$block_crawl[$key].time}</strong>{if !empty($block_crawl[$key].html_result)} {if !empty($block_crawl[$key].user_upd)}bởi: {$block_crawl[$key].user_upd}{/if} ({$block_crawl[$key].html_result}){/if}</div>
															</div>
														</div>
														<span class="fw-bold fs-20 text-warning">{$block_crawl[$key].total_stock}</span>
													</div>
												{else}
													<div class="d-flex gap-1 align-items-center">
														<div class="fs-11 d-flex flex-wrap gap-1 align-items-center lst_action_crawl">
															...
															<div class="d-flex gap-1 align-items-center flex-wrap gap-1">
																<div class="text-nowrap"><a href="{$clsISO->genGoogleURL($_oItem.spreadsheetId,'spreadsheets')}" target="_blank" class="limit_1line">Link cập nhật <i class='bx bx-link-external fs-11'></i></a></div>
																<div class="text-nowrap"><a href="{$block_crawl[$key].link_stock}" target="_blank" class="limit_1line text-main">Bảng hàng <i class='bx bx-link-external fs-11'></i></a></div>
																<div class="text-nowrap text-left w-100">Cập nhật: <strong class="text-main">{$block_crawl[$key].time}</strong>{if !empty($block_crawl[$key].html_result)} {if !empty($block_crawl[$key].user_upd)}bởi: {$block_crawl[$key].user_upd}{/if} ({$block_crawl[$key].html_result}){/if}</div>
															</div>
														</div>
