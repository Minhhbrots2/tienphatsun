<div class="modal-dialog modal-dialog-centered modal-fullscreen">
	<form method="POST" class="modal-content w-100 h-100">
		...
		<div class="modal-footer">					
			<div class="p__right d-flex gap-2 justify-content-end">
				<input type="hidden" uid="{$uid}" name="config_name" value="" />
				<select uid="{$uid}" class="slb_configs_{$uid} form-control iso-select2 js__do_config-change required" 
					onchange="$Core.money.do_change(this, event)" name="config_id" data-width="200px">
					<option value="0">Chọn cấu hình File</option>
					{if !empty($money_import_configs)}
						{foreach from=$money_import_configs key = _OK item = _oI}
						<option{if $_OK eq $config_id} selected="selected"{/if} value="{$_OK}">{$_oI.config_name}</option>
						{/foreach}
					{/if}
					<option value="_new">+ Thêm mới</option>
				</select>
				<button type="button" class="btn btn-primary" gId="{$gId}" onClick="$Core.money.do_config(this, event)">
