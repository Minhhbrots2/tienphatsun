...
function load_issue_globe(holderG, options){
	var $_adata = options || {};
	...
	// $Core.util.toggleIndicatior(1);
	$.post(PCMS_URL+'/index.php?mod='+MOD+'&act=load_issue_globe', $_adata, function(respJson){
		// $Core.util.toggleIndicatior(0);
		...
		if(parseInt(respJson.total_page) > 1){
			...
			$('#pager_'+holderG).removeClass('d-none').pagination({
				...
				onPageClick : function(pageNumber){
					load_issue_globe(holderG,$.extend(options,{'page':pageNumber}));
				}
			});
		} else {
		...
		$('#'+respJson.uid).on('shown.bs.modal', function(){
			...
			if($('.multiselect:not(.open_issue)').length){
				$('.multiselect:not(.open_issue)').addClass('open_issue').multiselect({
					...
					$.post(PCMS_URL+'/index.php?mod='+MOD+'&act=issue_participants', {
...
function pop_save_issue(_this, e){
	e.preventDefault();
	var _validated = 0,
		_form = $(_this).closest('form'),
		issue_id = $(_this).attr('issue_id'),
	...
	if(_validated==0){
		_form.ajaxSubmit({
			type:'POST',
			url: PCMS_URL + "/index.php?mod="+MOD+"&act=pop_save_issue",
			data: $_adata,
			success: function(html){
				if(html.indexOf('_success') >= 0){
					var tmp = html.split('|||');
					load_issue_globe(tmp[1], {});
					$Core.issue.load_issue_kanban("",{});
...
function load_issue_notes(issue_id, options) {
	var $_adata = options || {};
	$_adata['issue_id'] = issue_id;
...
function add_issue_note(_this, e){
	e.preventDefault();
...
function add_issue_task(_this, e){
	e.preventDefault();
...
function cancel_add_task(_this, e){
	e.preventDefault();
	var toId = $(_this).attr('toId'),
...
function save_add_task(_this, e){
	e.preventDefault();
	var _validated = 0,
		_form = $(_this).closest('form'),
		issue_id = $(_this).attr('issue_id'),
...
function load_issue_tasks(issue_id, options){
	var $_adata = options || {};
	$_adata['issue_id'] = issue_id;
...
function edit_issue_task(_this, e){
	e.preventDefault();
	var toId = $(_this).attr('toId');
...
function delete_issue_task(_this, e){
	e.preventDefault();
	var issue_id = $(_this).attr('issue_id'),
		issue_task_id = $(_this).attr('issue_task_id'),
...
function save_issue_pfield(_this, e) {
	e.preventDefault();
    var _form = $(_this).closest('form'),
		issue_id = $(_this).attr('issue_id');
    $Core.util.toggleIndicatior(1);
...
function open_issue_edit(_this, e){
	e.preventDefault();
	var toId = $(_this).attr('toId'),
...
function pop_save_issue_edit(_this, e){
	e.preventDefault();
	var toId = $(_this).attr('toId'),
		p_id = $(_this).attr('p_id'),
		p_field = $(_this).attr('p_field'),
...
function set_mention_list(_this, e){
	e.preventDefault();
	var gid = $(_this).attr('gid');
	$('#'+gid).toggleClass('d-none');
	return false;
}
function set_fireEvent(_this, e){
...
function issue_notes_upload(_this, e){
	e.preventDefault();
	var uid = $(_this).attr('uid');
	$('#issue_task_upload_file_'+uid).trigger('click');
	return false;
}
function issue_task_upload(_this, e){
	e.preventDefault();
	var toId = $(_this).attr('toId');
	$('#issue_task_upload_file_'+toId).trigger('click');
	return false;
}
$Core.issue = $.extend($Core.global.issue, {
	...
	archive: function(_this, e){
		...
		$.post('/index.php?mod='+MOD+'&act=archive_issue', $_adata, function(html) {
			$Core.util.toggleIndicatior(0);
			load_issue_globe(holderG, {'page':page});
		}, 'json');
		return false;
	},
	...
	load_issue_kanban: function(holderG,options) {
		var $_adata = options || {};
		$_adata['holderG'] = holderG;	
		if($('.search_field').length){
			$('.search_field').each((_i, _elem) => {
	...
	load_more_kanban: function(_this,e) {
		var $_adata = {};
