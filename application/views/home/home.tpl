<div class="content-wrapper">
	<div class="container-sm flex-grow-1 container-p-y">
		<div class="eznyDbxTuI mt-10">
			<nav class="navbar navbar-example navbar-light navbar-expand-lg gap-1 justify-content-center">
				<button class="btn btn-icon btn-outline-dark scroll-btn prev-btn d-lg-none">
					<i class='bx bx-chevron-left'></i>
				</button>
				<div id="project_tab" class="nav nav-pill navbar-nav flex-nowrap gap-2 text-nowrap overflow-x-auto" role="tablist">
					{foreach from=$list_tabs name= i key = _oKey item  = _oI}
					<a href="#{$_oI.slug}" onClick="$Core.project.tab_click(this, event)" slug="{$_oI.slug}" class="nav-item rounded-pill nav-link{if $smarty.foreach.i.first} active{/if} is-effect {$_oI.slug} {if $_oKey eq 'DN'}MRD{/if}" role="tab" data-bs-toggle="tab" data-bs-target="#{$_oI.slug}">{$_oI.title}</a>
					{/foreach}
				</div>
				<button class="btn btn-icon btn-outline-dark scroll-btn next-btn d-lg-none">
			...
			<div class="tab-content mt-3">
				...
				<div class="tab-pane fade{if $smarty.foreach.i.first} active show{/if}" id="{$_oI.slug}" role="tabpanel">
					{if $_oKey eq $smarty.const._BLOCK_TYPE_LOWFLOOR_SALE}
						...
						<div class="w-100 table-container overflow-auto" style="max-height: 700px">
					...
					{elseif $_oKey eq 'VIN'}
						...
						{if !empty($list_projects)}
							{foreach from=$list_projects item = _oProject}
								...
								<div class="w-100 table-container table-container-{$_oI.cls} overflow-auto mb-2 mt-3" style="max-height: 700px">
					...
					{elseif $_oKey eq 'DN' || $_oKey eq $smarty.const._PROJECT_CSD_ID}
						...
						{if !empty($list_projects)}
							{foreach from=$list_projects item = _oProject}
								{assign var = list_blocks value = $_oProject.list_blocks}
								{if $_oProject.total_stocks gt '0'}
								<div class="w-100 table-container table-container-{$_oI.cls} table-container-{$_oProject.code|lower} overflow-auto mb-2 mt-3" style="max-height: 700px">
									{if !empty($list_blocks)}
									<table border="0" cellpadding="0" cellspacing="0" class="table mb-0 table-stock-{$_oProject.code|lower}" width="100%">
									{foreach from=$list_blocks item = _oBlock}
										{assign var = block_id value = $_oBlock.property_id}
										{assign var = list_buildings value = $_oBlock.list_buildings}
										{assign var = list_price_field value = $_oBlock.list_price_field}
					...
					{else}
						...
						<div class="w-100 table-container table-container-{$_oI.cls} overflow-auto mb-2" style="max-height: 700px">
							<table border="0" cellpadding="0" cellspacing="0" class="table mb-0 table-stock-{$oneBlock.for_id} table-stock-{$_oI.cls}" width="100%">
								<thead class="sticky top-0" style="z-index: 100"><tr>
									<th class="pheader text-left text-upper" colspan="20">
										<div class="d-flex align-items-center justify-content-between">
						...
						tp="{$tp}" img_layout="{$_oI.img_layout}" class="map mt-3">
							<!-- Map -->
						</div>
						{/if}
						{/if}
