<div class="modal-dialog modal-dialog-centered modal-{if $p_field eq 'content' || $p_field eq 'participants'}ipad{else}sm{/if}">
	<form method="POST" enctype="multipart/form-data" class="modal-content">
		...
		<div class="modal-body">
			...
			{elseif $p_field eq 'title'}
			<div class="form-group">
				<label class="form-label">Nội dung công việc</label>
				<input type="text" name="title" class="form-control required fw-bold autofocus" value="{$oneIssue.title}" />
			</div>
		...
		<div class="modal-footer">
			...
			<button type="button" p_id="{$p_id}" p_field="{$p_field}" toId="{$toId}" onClick="$Core.issue.pop_save_issue_edit(this, event)" class="btn btn-primary">Lưu lại</button>
		</div>
	</form>
</div>