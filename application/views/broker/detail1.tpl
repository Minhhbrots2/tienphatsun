{if ($action eq 'preview' && $template eq '1') || ($action eq '' && $current_template eq '1')}	
	<div class="page_detail_broker">
		{if empty($is_profile_domain)} 
		<div class="container-xxl flex-grow-1 detail_broker detail_broker_theme_1 pt-2 text-dark">
			<div class="row">
				<div class="col-12 col-lg-12 col-xxl-12 col-xxxl-10 mx-auto">
					<div class="bg-broker rounded-3 overflow-hidden">
		{/if}
		<section class="banner_profile hero d-flex rounded-3 overflow-hidden position-relative bg-broker {if !empty($is_profile_domain)}justify-content-end{/if}" style="height: 400px">
			...
			{else}
				{if !empty($loggedIn) && $profile_id eq $oneItem.profile_id}
					<div class="btn-group position-absolute gap-1 right-10 top-10 zindex-1" style="top: 10px">
						...
						{/if}
						<a class="btn btn-icon btn-outline-default rounded-3" href="javascript:void(0)" onMouseMove="$Core.broker.setTooltip(this,'Sao chép link')" onclick="$Core.broker.copyToClipboard(this, event)" title="Sao chép link" data-link="{$clsProfile->getLink($oneItem.profile_id,$oneItem)}" ><i class="bx bx-link mr-1"></i></a>
			...
			{/if}
			{if !empty($is_profile_domain)} 
			<div class="col-auto col-xxxl-2 col-xxl-1 col-xl-1"></div>
			{/if}
			<div class="hero-content d-flex align-items-start {if $deviceType ne 'computer'} justify-content-between gap-3 px-2 py-4{else} p-4 flex-column justify-content-center{/if}">
				<div class="position-relative {if $deviceType eq 'phone'}w-px-100 h-px-100{else}w-px-150 h-px-150{/if} p-1 border bg-white rounded-pill mb-3">
					...
					{if !empty($more_information.image_certificate)}
						<span class="level cursor-pointer" data-fancybox="image_certificate" data-src="{$more_information.image_certificate}" title="Chứng chỉ môi giới" ><svg class="" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<div class="bg_banner h-100 position-relative flex-fill" style="background-image: URL('{if !empty($more_information.banner)}{$more_information.banner}{else}{$URL_IMAGES}/banner_default.jpg{/if}');background-size: cover">		
				{if !empty($more_information.dictum_live)}
					<div class="content_dictum_live fs-2 pe-4 mt-n1 position-absolute bottom-0 right-0 text-right w-80 text-white" style="text-shadow: 3px 1px black">{$more_information.dictum_live|html_entity_decode|strip_tags}</div>
				{/if}
			</div>
		</section>	
		{if empty($is_profile_domain)}</div></div></div></div>{/if}	
		{if !empty($is_profile_domain)}<div class="bg-broker2">{/if}	
			<div class="container-xxl flex-grow-1 detail_broker detail_broker_theme_1 text-dark">
				<div class="row">
					<div class="col-12 col-lg-12 col-xxl-12 col-xxxl-10 mx-auto">					
						{if empty($is_profile_domain)}<div class="bg-broker2">{/if}
						...
						{if empty($is_profile_domain)}</div>{/if}
					</div>	
				</div>	
		{if !empty($is_profile_domain)}</div>{/if}	
		...
		<div class="container-xxl flex-grow-1 detail_broker detail_broker_theme_1 text-dark">
			<div class="row">
				<div class="col-12 col-lg-12 col-xxl-12 col-xxxl-10 mx-auto {if !empty($is_profile_domain)}px-0{/if}">
					<div class="bg-broker overflow-hidden">	
												...
												<div class="info-container">
													...
													<ul class="list-unstyled mb-4 mt-3">
														{if !empty($oneItem.birthday)}
															<li class="d-flex align-items-center mb-2"><i class="bx bx-cake fs-5"></i><span class="fw-semibold mx-2 text-nowrap">Ngày sinh:</span> <span class="fs-6">{$oneItem.birthday}</span></li> 
														{/if}
										...
										{if $deviceType ne 'phone'}
											...
											{if !empty($lstShare)}
												<div class="card mb-2 no-shadow bg-broker2">
													...
													<div class="card-body">
														<div class="form-row row">
															{foreach from=$lstShare item=_oItem name=i}
																{if $smarty.foreach.i.index eq 6}{break}{/if}
																{assign var=lstImage value=$_oItem.lstImage}
																<div class="item col-4 mb-2">
																	<div class="rounded img_scale" data-fancybox="gallery_share" href="{$clsISO->getUrlImageFH($_oItem.image)}" data-caption="{$_oItem.title}">
																		<img class="drag-item cursor-pointer" src="{$clsISO->getUrlImageFH($_oItem.image,230,190)}" alt="avatar" style="width: 100%;height:94px">
																	</div>
																</div>
															{/foreach}
														</div>
												...
												<div class="card mb-2 no-shadow bg-broker2">
													...
													<div class="card-body">
														<div class="form-row row">
															...
															{section loop=$lstImage name=i max=6}
																<div class="item col-4 mb-2">
											...
											<div class="card no-shadow bg-broker2 mb-2">
									...
									<div class="box_content_broker mb-4 h-100">
										...
										{if !empty($more_information.about) || !empty($more_information.certificate) || !empty($more_information.working_process)} 
											...
											<div class="tab-pane fade show active {if $deviceType ne 'phone'}h-100{/if}" id="pills-about" role="tabpanel" aria-labelledby="pills-about-tab">
																	...
																	{foreach from=$more_information.certificate item=item key=key}
																		<div class="col mb-2">
																			<div class="box_item_certificate relative">
																				<div class="image_certificate img_scale" data-fancybox="gallery-certificate" data-src="{$item.image}" data-caption="{$item.title}"><img src="{$item.image}" alt="" width="200" height="260"></div>
												...
												{if !empty($more_information.customer_review) && $deviceType ne "phone"}
													{assign var=lstReview value=$more_information.customer_review}
													<section class="section_timeline mb-4 no-shadow pt-3">
														<div class="d-flex justify-content-between align-items-center box_head mb-3 gap-4">
															<hr class="flex-fill" style="color: #33509e;height: 2px">
															<h2 class="card-title mb-0 text-nowrap fs-4 text-dark text-upper">Nhận xét của khách hàng</h2>
															<hr class="flex-fill" style="color: #33509e;height: 2px">
														</div>	
														<div class="lstReview" data-lg-slide="3" data-md-slide="2" data-sm-slide="1" data-xs-slide="1" data-margin="5" data-nav="true" data-loop="true" data-autoplay="true">
															{foreach from=$lstReview item=_oItem name=i}
																<div class="item_review card no-shadow bg-broker2 rounded-3 mb-2">
																	<div class="card-header d-flex gap-2 align-items-center">
																		<div class="avatar-lg">
																			<img class="rounded-pill w-100 h-100 img-cover" src="{$_oItem.image}" alt="{$_oItem.customer_name}">
																		</div>
																		<div class="review__meta">
																			<h3 class="text-main2 fs-5 fw-bold mb-2">{$_oItem.customer_name}</h3>
																			<span class="text-muted">{$_oItem.date}</span>
																		</div>
																	</div>
																	<div class="card-body pt-0">
																		<div class="d-flex align-items-center mb-3 gap-2">
																			<span class="bg-warning px-3 py-1 rounded-3 text-white">{$_oItem.star}.0</span>
																			<span class="fw-bold text-warning fs-6">{$clsReview->getTextLevel($_oItem.star)}</span>
																		</div>
																		<div class="cus-desc">
																			<h3 class="fs-16 mb-2 text-main2 fw-bold">{$_oItem.title}</h3>
																			<div class="review__content review__content_short content_post">
																				<div class="content_full d-none">{$_oItem.content|html_entity_decode}</div>
																				{$clsISO->truncateWord($_oItem.content, 50,'... <a href="javascript:void(0)" onClick="$Core.broker.view_more(this,event)">Xem thêm</a>')}
																			</div>
																		</div>
																	</div>
																</div>	
										...
										{/if}
										{if !empty($lstShare)}
											<div class="share tab-pane fade {if $deviceType eq 'phone' || empty($check_active)} show active mb-2{else} h-100{/if}" id="pills-share" role="tabpanel" aria-labelledby="pills-share-tab"> 
												{assign var=check_active value=1}
												<div class="card no-shadow bg-broker2">
													<div class="card-body">
														<div class="dataTables_wrapper dt-bootstrap5 no-footer pt-3">
															...
															<div class="form-row row-cols-2 row-cols-lg-2 row-cols-xl-2">
																{foreach from=$lstShare item=_oItem}
																	<div class="col mb-2">
																		<div class="rounded img_scale position-relative h-100" data-fancybox="gallery1_share" href="{$clsISO->getUrlImageFH($_oItem.image)}" data-caption="{$_oItem.title}">
																			<img class="w-100 img-cover" src="{$clsISO->getUrlImageFH($_oItem.image,280,300)}" alt="" width="200" height="300">
																			<div class="position-absolute bottom-0 left-0 py-2 px-3 w-100" style="background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, rgba(0, 0, 0, 0.9) 100%)">
																				<h3 class="text-white mb-0 fs-6 {if $deviceType ne 'phone'}text-center{/if}">{$_oItem.title}</h3>
																			</div>
																		</div>
																	</div>
																{/foreach}
										...
										{/if}
										{if !empty($more_information.image)}
											<div class="share tab-pane fade {if $deviceType eq 'phone' || empty($check_active)} show active mb-2{else} h-100{/if}" id="pills-image" role="tabpanel" aria-labelledby="pills-image-tab"> 
												{assign var=check_active value=1}
												<div class="card no-shadow bg-broker2">
													<div class="card-body">
														<div class="dataTables_wrapper dt-bootstrap5 no-footer pt-3">
										...
										{if $deviceType eq 'phone'}
											...
											{if !empty($more_information.customer_review)}
												{assign var=lstReview value=$more_information.customer_review}
												<section class="section_timeline mb-4 no-shadow pt-3">
													<div class="d-flex justify-content-center align-items-center box_head mb-3 gap-4">
														<h2 class="card-title mb-0 text-nowrap fs-4 text-dark text-upper">Nhận xét của khách hàng</h2>
													</div>	
													<div class="lstReview" data-lg-slide="3" data-md-slide="2" data-sm-slide="1" data-xs-slide="1" data-margin="5" data-nav="true" data-loop="true" data-autoplay="true">
														{foreach from=$lstReview item=_oItem name=i}
															<div class="item_review card no-shadow bg-broker2 rounded-3 mb-2 px-3">
																<div class="card-header d-flex gap-2 align-items-center">
																	<div class="avatar-lg">
																		<img class="rounded-pill w-100 h-100 img-cover" src="{$_oItem.image}" alt="{$_oItem.customer_name}">
																	</div>
		...
		<div class="form-row">
			<div class="col-12 col-md-7 col-lg-8 col-xxl-8">
				<section class="section_banner relative">
					...
					{if !empty($more_information.dictum_live)}
						<div class="content_dictum_live fs-2 pe-4 mt-n1 position-absolute {if $deviceType eq 'computer'}bottom-0{else}top-0{/if} right-0 text-right text-white" style="text-shadow: 3px 1px black">{$more_information.dictum_live|html_entity_decode|strip_tags}</div>
					{/if}
				</section>
				<div class="card no-shadow box_content mb-2 content_scroll">
					<div class="card-body rounded-bottom d-flex flex-wrap justify-content-between align-items-end p-3 pb-2 gap-2">
						...
						<div class="box_right_head d-flex flex-wrap flex-fill">
							<div class="d-flex flex-wrap">
								...
								<a class="btn btn-icon btn-outline-primary" href="javascript:void(0)" onMouseMove="$Core.broker.setTooltip(this,'Sao chép link')" onclick="$Core.broker.copyToClipboard(this, event)" title="Sao chép link" data-link="{$clsProfile->getLink($oneItem.profile_id,$oneItem)}" ><i class="bx bx-link mr-1"></i></a>								
							</div>
						</div>	
					</div>
						...
						<div class="card no-shadow">
							<div class="card-body">
								<div class="dt-bootstrap5 no-footer">
									<div class="d-flex justify-content-between align-items-center box_head mb-3 gap-4">
										<h2 class="card-title mb-0 text-nowrap fs-4 text-dark">Nhận xét của khách hàng</h2>
									</div>
									<div class="box_body_video rounded">
										{foreach from=$lstReview item=_oItem name=i}
											<div class="item_review border rounded-3 mb-2">
												<div class="card-header d-flex gap-2 align-items-center">
													<div class="avatar-lg">
														<img class="rounded-pill w-100 h-100 img-cover" src="{$_oItem.image}" alt="{$_oItem.customer_name}">
													</div>
				...
				<section class="section_banner relative">
					<div class="d-flex justify-content-between align-items-center position-absolute w-100 p-2">				
						{if $action eq 'preview'}
							...
							{/if}
							<button class="btn btn-primary" type="button" onClick="$Core.broker.save_template(this,event)" template_id="{$template}">Lưu mẫu</button>
						{elseif empty($is_profile_domain)}
							<a href="{$clsISO->getLink('broker')}" class="btn_back"><i class='bx bxs-share'></i></a>
						{/if}
					</div>
					{if $deviceType eq 'phone'}
						<img class="w-100 rounded-top img-cover" src="{$more_information.banner}" onerror="this.src='{$URL_IMAGES}/banner_default.jpg'" alt="Banner" height="200"> 
					{else}
						<img class="w-100 rounded-top img-cover" src="{$more_information.banner}" onerror="this.src='{$URL_IMAGES}/banner_default.jpg'" alt="Banner" height="300"> 
					{/if}
					{if !empty($more_information.dictum_live)}
						<div class="content_dictum_live fs-2 pe-4 mt-n1 position-absolute {if $deviceType eq 'computer'}bottom-0{else}top-0{/if} right-0 text-right text-white" style="text-shadow: 3px 1px black">{$more_information.dictum_live|html_entity_decode|strip_tags}</div>
					{/if}
				</section>
				<div class="box_content {if !empty($is_profile_domain)}box_content_profile{/if}">
					<div class="box_content_top rounded-bottom d-flex flex-wrap justify-content-between p-3 pb-4 mb-3">
						<div class="box_left_head d-flex flex-wrap relative">
							<div class="box_image">
								<img class="rounded-circle w-100 h-auto" src="{$clsProfile->getAvatar($oneItem.profile_id,$oneItem,150,150)}" height="150" width="150" alt="User avatar" onerror="this.src='{$URL_IMAGES}/avatars/avatar.jpg'" style="object-fit: cover">
								{if !empty($more_information.image_certificate)}
								<span class="level cursor-pointer" data-fancybox="image_certificate" data-src="{$more_information.image_certificate}" title="Chứng chỉ môi giới" ><svg class="" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
									<g clip-path="url(#clip0_136:8783)">
									<path opacity="0.4" d="M13.2148 25.9395C20.1184 25.9395 25.7148 20.343 25.7148 13.4395C25.7148 6.53589 20.1184 0.939453 13.2148 0.939453C6.31128 0.939453 0.714844 6.53589 0.714844 13.4395C0.714844 20.343 6.31128 25.9395 13.2148 25.9395Z" fill="#16C784"/>
						...
						<div class="box_right_head d-flex flex-wrap justify-content-end">
							<div class="d-flex flex-wrap">
								<div class="me-2"><a href="mailto:{$oneItem.phone}" class="btn btn-outline-primary"><i class="bx bx-phone fs-5 me-2 align-text-bottom"></i>Gọi điện</a></div>
								<div class="me-2"><a href="mailto:{$oneItem.email}" class="btn btn-outline-primary"><i class="bx bx-envelope fs-5 me-2 align-text-bottom"></i>Email</a></div>
						...
						<div class="box_content_broker mb-2">
							<div class="" id="pills-about">
											...
											{if $deviceType eq 'phone'}
												{if $more_information.certificate|@count gt 1}
													<div class="box_certificate owl-carousel" id="owl-certificate">
														{foreach from=$more_information.certificate item=item key=key}
															<div class="box_item_certificate relative">
																<div class="image_certificate img_scale" data-fancybox="gallery-certificate" data-src="{$item.image}" data-caption="{$item.title}"><img src="{$item.image}" alt="" width="200" height="300"></div>
																<h3 class="title_certificate mb-0 p-3 text-white lh-sm fs-6">{$item.title}</h3>
															</div>
														{/foreach}
													</div>
											...
											{if $deviceType eq 'phone'}
												{if $more_information.project|@count gt 1}
													<div class="box_certificate owl-carousel" id="owl-da">
														{foreach from=$more_information.project item=item key=key}
															<div class="box_item_certificate item_da relative">
																<div class="image_certificate img_scale" data-fancybox="gallery-project" data-src="{$item.image}" data-caption="{$item.title}"><img src="{$item.image}" alt="" width="200" height="300"></div>
												...
												{else}
													<div class="box_certificate">
														{foreach from=$more_information.project item=item key=key}
															<div class="box_item_certificate item_da relative">
																<div class="image_certificate img_scale" data-fancybox="gallery-project" data-src="{$item.image}" data-caption="{$item.title}"><img class="w-100" src="{$item.image}" alt="" width="200" height="300"></div>
																<div class="title_certificate text-center px-3 py-2">
																	<h3 class="text-upper mb-1 lh-sm fs-6 text-white">{$item.title}</h3>
																	<p class="text-upper mb-0 lh-sm text-white fs-tiny">Giao dịch: {$item.total_sale_project}</p>
																</div>
													...
													{foreach from=$lstPostMeta item=oneMeta}
														<div class="item_post overflow-hidden border p-3 rounded-3 mb-2">
															...
															<div class="body_post">
																...
																<div class="content_post mb-3">
																	<div class="content_full d-none">{$oneMeta.content|html_entity_decode}</div>
												...
												{foreach from=$lstShare item=_oItem}
													<div class="item h-100">
														<div class="rounded img_scale position-relative h-100" data-fancybox="gallery1_share" href="{$clsISO->getUrlImageFH($_oItem.image)}" data-caption="{$_oItem.title}">
															<img class="w-100 img-cover" src="{$clsISO->getUrlImageFH($_oItem.image,280,300)}" alt="" width="200" height="300">
															<div class="position-absolute bottom-0 left-0 py-2 px-3 w-100" style="background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, rgba(0, 0, 0, 0.9) 100%)">
																<h3 class="text-white mb-0 fs-14">{$_oItem.title}</h3>
															</div>
														</div>
													</div>
												{/foreach}
								...
								{if !empty($more_information.customer_review)}
									...
									<div class="card no-shadow">
										<div class="card-body">
											<h2 class="text-upper rounded-pill bg-warning px-3 py-2 w-auto d-inline-block fw-bold text-dark box fs-6 mb-3 fst-italic">Nhận xét của khách hàng</h2>
											<div class="box_body_video rounded">
												{foreach from=$lstReview item=_oItem name=i}
													<div class="item_review border rounded-3 mb-2">
														<div class="card-header d-flex gap-2 align-items-center">
...
{/if}
{if !empty($is_profile_domain) && !empty($oneItem.phone)}
	
	<div class="hotline-phone-ring-wrap">
		<div id="zalo-widget" class="ww-normal zindex-4 ww-right ww-standard">
			<a title="Hỗ trợ Zalo" href="https://zalo.me/{$oneItem.phone}" target="_blank" class="ww-text" previewlistener="true"> 
				<span>Hỗ trợ Zalo</span>
				<div class="ww-arrow"></div>
			</a> 
			<div class="ww-icon">
				<a title="Zalo" class="ww-icon-link" target="_blank" href="https://zalo.me/{$oneItem.phone}" previewlistener="true"> 
					<img src="{$URL_IMAGES}/zalo_chat.png" width="40px">
				</a>
			</div> 
		</div>
		<div class="hotline-phone-ring">
			...
			<div class="hotline-phone-ring-img-circle">
				<a href="tel:{$oneItem.phone}" class="pps-btn-img">
					<img src="https://futurehomes.vn/wp-content/themes/houzez/images/phone-icon.png" alt="Gọi điện thoại" width="50">
				</a>
			</div>
		</div>
		<div class="hotline-bar">
			<a href="tel:097 103 3333">
				<span class="text-hotline">{$oneItem.phone}</span>
			</a>
		</div>
	</div>
{/if}