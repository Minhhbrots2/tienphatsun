<div class="modal-dialog modal-dialog-centered modal-fullscreen">
	<form method="POST" class="modal-content w-100 h-100">
		<div class="modal-header">
			<h3 class="modal-title"><strong>Cài đặt cột dữ liệu </strong></h3>
						...
						<tr>
							...
							<th class="bg-white" style="min-width:125px" width="{$widthColumn}%">
								<select name="columns[{$col}]" data-width="100%" class="form-control form-select stock_import_field">
									...
									<option value="{$key}" {$select_default[$smarty.section.index.index]} {if $select_default[$smarty.section.index.index] eq $key}selected{/if}>{$_title}</option>
									{/foreach}
								</select>
							</th>
							{/section}
		...
		<div class="modal-footer">					
			<div class="p__right d-flex justify-content-end">
				<button type="button" class="btn btn-primary" uid="{$uid}" onClick="$Core.quiz.continue_config(this, event)">
					<span>Tiếp tục</span>
				</button>
			</div>
		</div>
