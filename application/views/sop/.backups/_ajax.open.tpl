		...
		<div class="modal-header position-relative d-block">
			<div class="d-flex align-items-center justify-content-between">
				<div class="d-flex align-items-center">
					<h5 class="modal-title fs-{if $deviceType eq 'phone'}5{else}3{/if} text-main mr-2 text-upper">{if $more_information.sop_type ne $smarty.const._TYPE_HIGHLEVEL}{$stock_code}{else}{$stock_code} - {$clsProperty->getTitle($oneSop.bedroom_id)}{/if}</h5>
					<a href="javascript:void(0)" onClick="$Core.sop.copyToClipboard(this, event)" data-bs-toggle="tooltip" title="Đã sao chép link căn hộ" data-bs-trigger="click" data-link="{$PCMS_URL}{$clsSop->getLink($sop_id, $stock_code)}" class="sop_link text-dark rounded-pill fs-6">{$clsISO->makeIcon('bx-link')}</a>
				</div>
			...
			<div class="d-flex align-items-center justify-content-between">
				<div class="d-flex align-items-center p_left">
					<span class="text-muted fs-12 mr-1">
						<a onClick="$Core.sop.add_filter(this, event)" tp="user" user_id="{$oProfile.profile_id}" title="{$clsProfile->getFullName($oProfile.profile_id, $oProfile)}" full_name="{$clsProfile->getFullName($oProfile.profile_id, $oProfile)}" href="javascript:;"><img class="avatar rounded-pill avatar-xs" onerror="this.src='{$URL_IMAGES}/no-avatar.jpg'" src="{$clsProfile->getAvatar($oProfile.profile_id,$oProfile,30,30)}" alt="{$clsProfile->getFullName($oProfile.profile_id, $oProfile)}" /></a>
					</span>
		    ...
		    <div class="modal-body">
				<div class="rounded-2 mb-2{if $deviceType ne 'phone'} overflow-hidden{/if}">
					<div class="d-flex highlight tEzTuYtLau bg-grayter p-lg-2 mb-0">
						<div class=" text-center flex-fill">
							<p class="mb-0 text-muted lh-base">
								<i class="d-inline-block re__icon-house--sm"></i> Tòa/Dãy
							</p>
							<div class="d-flex align-items-center justify-content-center fw-bold text-main">
								{if $deviceType eq 'phone'}
									{$clsProperty->getTitle($oneSop.building_id)},
									{$clsProperty->getCode($oneSop.block_id)}
								{else}
									{$clsProperty->getTitle($oneSop.building_id)},
									{$clsProperty->getTitle($oneSop.block_id)}
								{/if}
								{if $total_shops gt '0'}
								<a class="text-main ml-1" data-bs-toggle="tooltip" title="Cửa hàng/dịch vụ, tiện ích" building_id="{$oneSop.building_id}" onClick="$Core.sop.open_shop(this, event)" href="javascript:void(0);"> <i class="material-icons-outlined no-translate">storefront</i></a>
								{/if}
							</div>
						</div>
						<div class=" text-center flex-fill">
					...
					<div class="position-relative">
						...
						<div id="{$clsISO->getUniqid()}" class="slideshow{if $total_medias gt '1'} owl-carousel{/if} mb-0">
							...
							<div class="slideshow-item w-100 h-100 position-relative overflow-hidden">
								...
								<div class="sop_slider_image d-flex justify-content-center position-relative zindex-2 cursor-zoomIn" data-fancybox="gallery" href="{$_oMedia.image}?v={$upd_version}">
									<img src="{$clsISO->resizeImageFromUrl($_oMedia.image,0,400)}?v={$upd_version}" alt="{$oneSop.stock_code}" />
								</div>
					...
					<div class="d-flex highlight tEzTuYtLau bg-grayter p-lg-2">
						{if $more_information.sop_type ne $smarty.const._TYPE_HIGHLEVEL}
						...
						<div class="d-flex flex-wrap justify-content-between">	
							{if !empty($oneSop.home_direction_id)}
								<div class="d-flex flex-wrap pr-2 fs-14 align-items-center d-lg-none">
									<p class="mb-0 text-muted lh-base"><i class="d-inline-block re__icon-ying-yang--xl"></i> Ban công: </p>
			...
			<div class="modal-footer">
				<div class="w-100 d-flex flex-wrap ox-sm:col-4 align-items-center gap-2{if $loggedIn ne '1'} oDimQHEQij{/if}">
					...
					<a href="tel:{$oneSop.contact_phone}" type="phone" sop_id="{$sop_id}" 
						...
						<i class="material-icons-outlined">call</i> {$clsProfile->mask($oneSop.contact_phone, 1)}
					</a>
					<a href="https://zalo.me/{$oneSop.contact_phone}" type="zalo" target="_blank" sop_id="{$sop_id}" 
						class="btn flex-fill btn-outline-default qqfSRjPFMr text-nowrap over px-2">
						<img src="{$URL_IMAGES}/zalo_chat.png" width="20px" /> 
						{if $deviceType eq 'phone'}
							{$clsISO->truncate($oneSop.contact_name,16)}
						{else}
							{$clsSop->getContactName($oneSop.contact_name)}
						{/if}
					</a>
					{if $loggedIn ne '1'}
						...
						{if $deviceType eq 'phone'}
						<a type="button" class="btn d-flex align-items-center justify-content-center flex-fill btn-outline-default qqfSRjPFMr" return_url="{$return_url}" onClick="$Core.sop.close_pop(this, event)" data-bs-dismiss="modal"><i class='bx bx-x fs-4'></i> Đóng cửa sổ</a>
						{else}
						<a type="button" class="btn btn-icon btn-outline-default qqfSRjPFMr" return_url="{$return_url}" onClick="$Core.sop.close_pop(this, event)" data-bs-dismiss="modal"><i class='bx bx-x fs-2'></i></a>
						{/if}
					{else}			
						{if $profile_id eq $oneSop.user_id}
							<a type="button" class="btn btn-icon btn-outline-default qqfSRjPFMr" return_url="{$return_url}" onClick="$Core.sop.close_pop(this, event)" data-bs-dismiss="modal"><i class='bx bx-x fs-2'></i></a>
						{else}
							{if $deviceType eq 'phone'}
							<a type="button" class="btn d-flex align-items-center justify-content-center flex-fill btn-outline-default qqfSRjPFMr" onClick="$Core.sop.close_pop(this, event)" return_url="{$return_url}" data-bs-dismiss="modal"><i class='bx bx-x fs-4'></i> Đóng cửa sổ</a>
							{else}
							<a type="button" class="btn btn-icon btn-outline-default qqfSRjPFMr" return_url="{$return_url}" onClick="$Core.sop.close_pop(this, event)" data-bs-dismiss="modal"><i class='bx bx-x fs-2'></i></a>
							{/if}
						{/if}
