<div class="modal-dialog modal-dialog-centered">
	<form class="modal-content" method="POST" enctype="multipart/form-data">
		<div class="modal-header border-bottom">
			{if !empty($need_id)}
				<h5 class="modal-title">Sửa nhu cầu</h5>
			{else}
				<h5 class="modal-title">Thêm nhu cầu</h5>
			{/if}
		...
		<div class="modal-body">
					...
					<div class="d-flex align-items-center gap-2 justify-content-start">
						<div class="d-flex align-items-center gap-1 box_number_bedroom">
							<label class="we-radio" for="bedroom_{$uid}_1">
								...
								<span class="lbl_text {if $oneItem.bedroom_num gt 5}cursor-not-allowed{/if}">1</span>
							</label>
							<label class="we-radio" for="bedroom_{$uid}_2">
								...
								<span class="lbl_text {if $oneItem.bedroom_num gt 5}cursor-not-allowed{/if}">2</span>
							</label>
							<label class="we-radio" for="bedroom_{$uid}_3">
								...
								<span class="lbl_text {if $oneItem.bedroom_num gt 5}cursor-not-allowed{/if}">3</span>
							</label>
							<label class="we-radio" for="bedroom_{$uid}_4">
								...
								<span class="lbl_text {if $oneItem.bedroom_num gt 5}cursor-not-allowed{/if}">4</span>
							</label>
							<label class="we-radio" for="bedroom_{$uid}_5">
								...
								<span class="lbl_text {if $oneItem.bedroom_num gt 5}cursor-not-allowed{/if}">5</span>
							</label>
						</div>
						<div class="input-group flex-fill">
							<input class="form-control no-focus w-100 input_no_outer_spin" type="number" placeholder="Nhập số" onChange="$Core.crm.loadNumberRadio(this,event)" data-field="bedroom" name="bedroom_num" value="{if $oneItem.bedroom_num gt 5}{$oneItem.bedroom_num}{/if}">
						</div>
					</div>
				</div>
					...
					<div class="d-flex align-items-center gap-2 justify-content-start">
						<div class="d-flex align-items-center gap-1 box_number_bathroom">
							<label class="we-radio" for="bathroom_{$uid}_1">
								...
								<span class="lbl_text {if $oneItem.bathroom_num gt 5}cursor-not-allowed{/if}">1</span>
							</label>
							<label class="we-radio" for="bathroom_{$uid}_2">
								...
								<span class="lbl_text {if $oneItem.bathroom_num gt 5}cursor-not-allowed{/if}">2</span>
							</label>
							<label class="we-radio" for="bathroom_{$uid}_3">
								...
								<span class="lbl_text {if $oneItem.bathroom_num gt 5}cursor-not-allowed{/if}">3</span>
							</label>
							<label class="we-radio" for="bathroom_{$uid}_4">
								...
								<span class="lbl_text {if $oneItem.bathroom_num gt 5}cursor-not-allowed{/if}">4</span>
							</label>
							<label class="we-radio" for="bathroom_{$uid}_5">
								...
								<span class="lbl_text {if $oneItem.bathroom_num gt 5}cursor-not-allowed{/if}">5</span>
							</label>
						</div>
						<div class="input-group flex-fill">
							<input class="form-control no-focus w-100 input_no_outer_spin" type="number" placeholder="Nhập số" onChange="$Core.crm.loadNumberRadio(this,event)" data-field="bathroom" name="bathroom_num" value="{if $oneItem.bathroom_num gt 5}{$oneItem.bathroom_num}{/if}">
						</div>
					</div>
				</div>
			</div>
			<div class="form-group mb-2">
				...
				<textarea id="textarea" class="form-control no-focus" maxlength="300" rows="5" name="content" placeholder="Nhập đặc điểm mô tả ... ">{$oneItem.content}</textarea>
				<div class="form-text">Tối đa 300 ký tự</div>
			</div>
			<div class="form-group mb-2">
				<div class="form-check-reverse">
					<label class="form-check-label me-2" for="is_hot_{$uid}">Nhu cầu HOT</label>
					<input class="form-check-input" type="checkbox" name="is_hot" value="1" id="is_hot_{$uid}" {if $oneItem.is_hot eq 1}checked{/if} >
				</div>
			</div>
		</div>
		<div class="modal-footer">
			...
			<button class="btn btn-primary" type="button" toId="{$toId}" 
				onClick="$Core.crm.addNeed(this, event)" customer_id="{$customer_id}" >Lưu lại</button>
		</div>
	</form>
</div>