...
function default_default(){
	...
	$list_status_array = $clsProperty->getAllCache("`is_trash`=0 AND `property_type`='CUSTOMER_STATUS' 
...
function default_load_desktop_followups(){
	...
	$html= ''; $total_record = 0; // $dbconn->debug = true;
...
function default_save_activity(){
	global $oSmarty,$smarty,$profile_id,$core,$clsISO,$oneProfile;
	$clsNotify = new Notify();
	$clsProfile = new Profile();
	$clsProperty = new Property();
	$clsCustomer = new Customer();
	...
	$customer_id = (int) Input::post('customer_id', 0);
	$oCustomer = $clsCustomer->getOne($customer_id, "`admin_id`,`list_share_id`,`status_id`,`more_information`,`name`,`notes`");
...
function default_update_field(){
	...
	if($p_field=='tags'){
		...
		$oCustomer = $clsCustomer->getOne($p_id, "`name`,`admin_id`,`list_share_id`,`more_information`");
		$admin_id = (int) $oCustomer['admin_id'];
		$list_share_id = $oCustomer['list_share_id'];
		$more_information = $oCustomer['more_information'];
		$more_information = $clsISO->to_array_json($more_information);
			...
			if( $clsCustomer->updateOne($p_id, array(
				...
				if($clsCustomer->isRootProfile()){
					...
					if(!empty($zaloId)){
						...
						$message.= "\r";
						$message.= sprintf("[%s %s] đã giao cho bạn phụ trách khách hàng [%s]", 
							$oneProfile['role_name'], $clsProfile->getFullName($profile_id, $oneProfile),
							$clsCustomer->getName($p_id, $oCustomer));
						$message.= "\r";
		...
		if($p_value != $oCustomer['status_id']){
			...
			$content = sprintf('<strong>%s</strong> đã cập nhật tình trạng khách hàng từ <strong>%s</strong> tới <strong>%s</strong>', 
...
function default_load_conversion_rate(){
	...
	foreach($arr_status as $key => $val) {
		...
		if($key == _CRM_STATUS_TRASH_ID || $key == _CRM_STATUS_LONG_TERM_LEAD_ID) {
			$dataPoints[] = array(
				'label'	=> $val["title"],
				'y'	=> round(($total_cus > 0) ? ($total * 100 / $total_cus) : 0,1),
				'total_to'	=>	$total,
				'total_from' =>	$total_cus,
			);
		}else{
...
function default_view_billing(){
	...
	$lstBillings = !empty($more_information['lstBillings']) ? $more_information['lstBillings'] : array();
...
function default_list_logs(){
	...
	$logs = !empty($more_information['lstBillings']) ? $more_information['lstBillings'] : array();;
...
function default_save_notes(){
	...
	$more_information['lstBillings'] = $lstBillings;
	if($clsCustomer->updateOne($customer_id, array(
		'more_information' => json_encode($more_information, JSON_UNESCAPED_UNICODE)
	))){
		$msg = '_success';		
	...
	foreach ($list_staffs as $key => $val) {
		...
		if(!empty($list_customers)){
			...
			foreach ($list_customers as $k_cus => $v_cus) {
				...
				$more_information = $clsISO->to_array_json($more_information);
				$list_billings = !empty($more_information['lstBillings']) ? $more_information['lstBillings'] : array();
...
function default_load_employ_followup(){	
	...
	$clsISO->print_pre($lstFollowUps);die;	
...
//	$clsISO->print_pre($lstCustomer);die;
	...
	foreach ($lstCustomer as $key => $val) {
		...
		if(!empty($more_information["lstBillings"])) {
			foreach ($more_information["lstBillings"] as $k => $v) {
...
function default_backfill_customer_relations(){
	...
	if(!$clsCustomer->relationTableExists($clsCustomerShare->tbl) || !$clsCustomer->relationTableExists($clsCustomerCampaign->tbl)){
		echo "MISSING_RELATION_TABLES\n";
		echo "Run scripts/migrations/2026-04-14_customer_relations.sql first.\n";
		die();
	}
	$list = $clsCustomer->getAll("1=1", "{$clsCustomer->pkey},list_share_id,list_campaign_id,user_id");
	$totalCustomers = 0;
	$totalShareRows = 0;
	$totalCampaignRows = 0;
	if(!empty($list)){
		foreach($list as $idx => $row){
			...
			$shareIds = $clsCustomer->normalizeIdArray($row['list_share_id']);
			$campaignIds = $clsCustomer->normalizeIdArray($row['list_campaign_id']);
			$clsCustomer->syncShareIds($customer_id, $shareIds, $user_id, false);
			$clsCustomer->syncCampaignIds($customer_id, $campaignIds, $user_id, false);
			$totalCustomers++;
			$totalShareRows += count($shareIds);
			$totalCampaignRows += count($campaignIds);
	...
	echo "share_rows={$totalShareRows}\n";
	echo "campaign_rows={$totalCampaignRows}\n";
	die();
}
?>
