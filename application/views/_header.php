<?php
	...
	$takeleave_configs,$_frontIsLoggedin,$clsCategory,$clsCookie,$clsISO,$deviceType,$clsSetting,$extLang;
	#
	$extLang = ''; 
	...
	if($loggedIn){
		...
		#profile MOC
		if($clsISO->checkDEV()) {
			$oneMember = $clsMember->getByCond("`email`='".$oneProfile['email']."'","full_name_slug");
			if(!empty($oneMember)) {				
				$link_profile_sale = "https://profile.futurehomes.vn/".$oneMember["full_name_slug"];
				$assign_list["link_profile_sale"] = $link_profile_sale;
			}
		}
	} else {
		if($mod !='auth'){
