		...
		<div class="d-flex justify-content-between align-items-start mb-2">
			...
			{if !empty($arr_projects)}
				<div class="btn-group">
					...
					<button type="button" class="btn btn-outline-default btn-icon dropdown-toggle hide-arrow" data-bs-toggle="dropdown" aria-expanded="false"><i class="bx bx-filter-alt"></i></button>
					{else}
					<button type="button" class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">Chọn dự án</button>
					{/if}
					<ul class="dropdown-menu">
						{foreach from=$arr_projects item=_oItem key=key name=i}
							{if $_oItem.project_id eq $project_id && $_oItem.block_id eq $block_id }
								<li><a class="dropdown-item active" href="javascript:void(0)">{$_oItem.block_name}</a></li>
							{else}
								<li><a class="dropdown-item " href="{$clsBooking->getLinkBooking($_oItem.project_id,$_oItem.block_id)}">{$_oItem.block_name}</a></li>
							{/if}
						{/foreach}
					</ul>
				</div>
			...
			<div class="card-body">
				<div class="freeze-table text-nowrap">
					<table class="table fixedTable table-stock table-bordered border-label-dark">
						<thead>
							<tr>
								<th class="align-center text-black text-center h-px-50" colspan="{$more_building.template|@count + 2}" style="min-width:60px;background:#f5ea57">
								<strong class="fs-24 text-upper">Bảng booking ưu tiên Tòa {$_oBuilding.title}</strong>
								</th>
							</tr> 
							<tr style="background:#f5b376">
								<th class="text-black text-center h-px-30" width="60px" colspan=""  style="background: inherit">Khoảng <br> tầng/Căn</th>
								<th class="text-black text-center h-px-30" width="60px" rowspan="">Ưu tiên</th>
								{foreach from=$more_building.template item=_oTemplate key=k_temp name=i_temp}
									<th width="60px" class="text-center js__stock-cell-focus text-black h-px-30" code="{$_oTemplate.code}" >{$_oTemplate.code}</th>
								{/foreach}
							</tr>
							<tr style="background: #fff2d0">
								<th class="text-black text-center h-px-30" colspan="" style="background: #f3cccf">L. Căn</th>
								<th class="text-black text-center h-px-30" colspan=""></th>
								{foreach name=k from=$arr_stocks item = _code}
								{assign var = beroom_id value = $clsProject->getFieldInCol($_code,$arr_cols,'bedroom_id')}
								<th class="text-center js__stock-cell-focus text-black" code="{$_code}" style="background:{$bedroom_bgcolor_arrs.$beroom_id}">{$clsProject->getFieldInStock($_code, $arr_cols, 'bedroom_id')}</th>
								{/foreach}
							</tr>
							<tr>
								<th class="text-black text-center h-px-30" colspan="" style="background: #f3cccf">DT.Tim</th>
								<th class="text-black text-center h-px-30" colspan=""></th>
								{foreach name=k from=$arr_stocks item = _code}
								{assign var = beroom_id value = $clsProject->getFieldInCol($_code,$arr_cols,'bedroom_id')}
								<th class="text-center js__stock-cell-focus text-black" code="{$_code}" style="background:{$bedroom_bgcolor_arrs.$beroom_id}">{$clsProject->getFieldInStock($_code, $arr_cols, 'DT_Tim')}</th>
								{/foreach}
							</tr>
							<tr>
								<th class="text-black text-center h-px-30" colspan="" style="background: #f3cccf">DT.TT</th>
								<th class="text-black text-center h-px-30" colspan=""></th>
								{foreach name=k from=$arr_stocks item = _code}
								{assign var = beroom_id value = $clsProject->getFieldInCol($_code,$arr_cols,'bedroom_id')}
								<th class="text-center js__stock-cell-focus text-black" code="{$_code}" style="background:{$bedroom_bgcolor_arrs.$beroom_id}">{$clsProject->getFieldInStock($_code, $arr_cols, 'DT_TT')}</th>
								{/foreach}
							</tr>
						</thead>
						{foreach from=$layout_range item=ranges key=k_layout name=i_layout}	
								{foreach from=$ranges item=_range key=k_range name=i_range}	
									<tbody style='background:{cycle values="#fff2ce,#f3cccf"}' class=''>
											...
											{section loop=$row_ut start=0 step=1 name=row}
												<tr>
													<td class="text-center h-px-30">{$smarty.section.row.iteration}</td>
													{foreach from=$more_building.template item=_oTemplate key=key name=i}
														...
														<td style="background:#5dff00;" code="{$_oTemplate.code}" floor="38" class="js__stock-cell-focus text-center">
															<span class="text-nowrap" booking_id="{$_oBooking.booking_id}" booking_type="{$_oBooking.booking_type}">{$oStaff.department_name}</span>
														</td>
														{else}
															<td code="{$_oTemplate.code}" floor="38" class="js__stock-cell-focus">
															</td>
														{/if}
													{/foreach}
												</tr>
