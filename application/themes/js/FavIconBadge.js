...
class FavIconBadge extends HTMLElement {
    constructor() {
        super();
        ...
        this.canvas.width = FavIconBadge.favIconSize;
        this.canvas.height = FavIconBadge.favIconSize;
    ...
    updateIcon() {
        ...
        context.clearRect(0, 0, FavIconBadge.favIconSize, FavIconBadge.favIconSize);
        context.drawImage(this.image, 0, 0, FavIconBadge.favIconSize, FavIconBadge.favIconSize);
        const badgeSize = this.badgeSize;
        if (shouldDrawBadgeForContent(this.badge)) {
            if (this.badgeBackgroundSrc) {
                context.drawImage(
					this.badgeBackgroundImage, 
            ...
            if (shouldDrawTextForContent(this.badge)) {
                ...
                context.fillText(
					this.badge.slice(0, 2), 
					this.canvas.width - badgeSize / 2, 
					this.canvas.height - badgeSize / 2
				);
            }
        }
        this.link.href = this.canvas.toDataURL('image/png');
    }
    drawBadgeCircle(to, color, size) {
...
FavIconBadge.favIconSize = 16;
customElements.define('favicon-badge', FavIconBadge);
//# sourceMappingURL=FavIconBadge.js.map
