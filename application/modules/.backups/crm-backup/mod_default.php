...
function default_load_customers(){
	...
	if($deviceType == 'phone'){
		...
		if(!empty($list_customers)){ $ii=0;// Init
			...
			foreach($list_customers as $cus){
				...
				if(!isset($arr_property_cached[$status_id])){
					$arr_property_cached[$status_id] = $clsProperty->getLabel($status_id, " mr-1", true);
				}
				$html_follow_ups = "";
				...
				if(!empty($list_follows_ups)){ $kk = 1;
					...
					foreach($list_follows_ups as $nkey => $nval){
						$html_follow_ups.= '<li class="fs-12 my-0"><span class="text-'.($nval['date_id'] > $now ? 'main' : 'primary').'">'.($nval['date_id'] > $now ? $clsISO->getTimeMore($nval['date_id']) : $clsISO->getTimeAgo($nval['date_id']) ) .'</span> - '.ucfirst($nval['intro']).'</li>';
						++$kk; 
					}
					$html_follow_ups.= '</ul>';
				}
				$html.= '<div class="awe__customer-item border-bottom pointer-event py-2" onDblClick="$Core.crm.view_activity(this, event);" customer_id="'.$customer_id.'">
					<div class="d-flex align-items-center justify-content-between">
						<div class="d-flex align-items-center">
							'.$arr_property_cached[$status_id].'
							<a href="javascript:void(0);" onClick="$Core.crm.open_customer(this,event)" class="link goLink font-bold view_customer text-nowrap" route="/customer/'.$customer_id.'/overview" '.$props.'>'.ucfirst($cus['name']).'</a>
						</div>
						<div class="btn-group">
					...
					<div class="d-flex justify-content-between mb-1 align-items-center">
						<div class="d-flex align-items-center">
							<a href="javascript:void(0);" onClick="$Core.crm.change_assigned(this, event)" 
								customer_id="'.$customer_id.'" class="user-avatar">
								<img class="avatar avatar-xxs rounded-pill" src="'.$clsProfile->getAvatar($admin_id).'">
								<span class="cre">'.$core->makeIcon($icon).'</span>
							</a> 
							<span class="d-flex gap-1 align-items-center text-muted fs-12">
					...
					'.(!empty($title_purpose) ? '<div class="d-flex my-1 align-items-center">
						<span class="text-muted fs-12">Mục đích:</span>'.$title_purpose.'
					</div>': '').'
					'.(!empty($html_follow_ups) || !empty($html_tags) || !empty($html_stocks) ? '<div class="d-block">
		...
		<table border="0" cellpaddin="0" cellspacing="0" class="table dragable mb-0" width="100%">
			<thead><tr>
				<th class="align-center bg-lighter text-center h-px-40" width="40px">
					<input type="checkbox"'.($is_checkbox==false ? ' disabled' : '').' 
						class="form-check-input" tp="all" onChange="$Core.crm.check_item(this, event)" />
				</th>'. ($view_by=='table' ? '
				<th class="align-center bg-lighter h-px-40" style="width:15%">Họ và tên</th>
				<th class="align-center bg-lighter h-px-40" width="150px">Điện thoại</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Nguồn khách</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Chiến dịch</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Loại hình</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Tình trạng</th>
				<th class="align-center text-left bg-lighter h-px-40" width="50px">Q.Lý</th>
				<th class="align-center text-left bg-lighter h-px-40" colspan="2">Follow-Ups</th>
				<th class="align-center text-center bg-lighter h-px-40" width="60px"></th>' : '
				<th class="align-center text-left bg-lighter h-px-40" width="130px">Ngày</th>
				<th class="align-center text-left bg-lighter h-px-40">Họ và tên</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Điện thoại</th>
				<th class="align-center text-center bg-lighter h-px-40" width="50px">Q.Lý</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Tình trạng</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Nguồn khách</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Loại hình</th>
				<th class="align-center text-left bg-lighter h-px-40" width="120px">Chiến dịch</th>
				<th class="align-center text-left bg-lighter h-px-40" width="20%">Mô tả</th>
				<th class="align-center text-center bg-lighter h-px-40" width="60px"></th>').'
			</tr></thead>';
			if(!empty($list_customers)){ $ii=0;// Init
				$arr_profile_cached = $arr_property_cached = $arr_status_cached = array();
				foreach($list_customers as $cus){
					...
					if($view_by == 'table'){
						$html .= '<tr class="pointer-event" onDblClick="$Core.crm.view_activity(this, event);" customer_id="'.$customer_id.'" class="trCustomer">
							...
							<td class="text-left">
								<div class="d-block text-nowrap"><a href="javascript:void(0);" onClick="$Core.crm.open_customer(this,event)" class="link goLink font-bold view_customer fs-6" route="/customer/'.$customer_id.'/overview" '.$props.'>'.(!empty($cus['name']) ? ucfirst($cus['name']) : "Không tên") .'</a>'.$title_purpose.'</div>
							...
							<td class="text-left text-nowrap">'.$arr_profile_cached[$resource_id].'</td>
							<td class="text-left text-nowrap">'.(!empty($campaign_arrs) ? $clsCampaign->getTitleArray($campaign_arrs) : "--").'</td>
							<td class="text-left text-nowrap">'.$blocktype_name.'</td>
							<td class="text-left">'.$arr_property_cached[$status_id].'</td>
							<td data-label="Gán cho" class="text-center">
								<a href="javascript:void(0);"'.($admin_id==$profile_id?' onClick="$Core.crm.change_assigned(this, event)"':'').' customer_id="'.$customer_id.'" class="user-avatar" data-url="/index.php?mod=home&amp;act=load_profile_popover&user_id='.$admin_id.'" data-toggle="webui-popover" data-trigger="hover" data-width="350">
	...
	$html.= '<input type="hidden" class="PageCustomer_Length" value="'.$per_page.'" />
	...
	if(!empty($list_status_array)){ $ii = 1;
		$font_size = ($deviceType=='phone') ? 13 : 14;
		foreach($list_status_array as $key => $val){
			...
			$html_briefs.= '<div data-toggle="ripple" class="brief-item flex-fill'.($deviceType=='phone' && $ii>6 ? ' d-none' : '').' cursor-pointer m-0 overflow-hidden" onClick="$Core.crm.set_status(this,event)" status_id="'.$property_id.'" style="background:'.$val['bgcolor'].'">
				<p class="fs-'.$font_size.' text-white text-nowrap mb-1">'.$val['title'].'</p>
				<h3 class="fs-5 mb-0 text-white">'.$total_customers.'</h3>
			</div>';
			++$ii;
		}
	}
		...
		if(@is_uploaded_file($_FILES['fileimport']['tmp_name'])){
			...
			if (move_uploaded_file($_FILES["fileimport"]["tmp_name"], $target_file)) {
				...
				if(!empty($tblData)){
					...
					if(!empty($totalInsert)) {
						#activity log			
						$clsActivityLog = new ActivityLog();
						$log = $clsActivityLog->addActivityLog("Customer","insert");
					}
				}
			}
		}
