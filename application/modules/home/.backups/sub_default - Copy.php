...
function default_default(){
	...
	if(!empty($lstHomeGratitude)){
		...
		foreach($lstHomeGratitude as $key => $val){
			...
			if(!empty($scoreEmp)) {
				$lstStaff = $clsProfile->getAllCache("profile_id IN (".implode(",",array_keys($scoreEmp)).")",$clsProfile->pkey.',full_name');
	...
	$lstHomeNews = $clsNews->getAll("is_trash=0 and cat_id <> '"._NEWS_GRATITUDE_CAT_ID."' order by reg_date DESC limit 0,3");
...
function default_cancel_billing(){
	...
	$billing_id = Input::post('billing_id', 0);
	###
	$oneBilling = $clsBilling->getOne($billing_id, "is_cancel,logs");
	if($oneBilling['is_cancel']==1){
		...
		$logs = !empty($logs) ? @json_decode(html_entity_decode($logs), true) : array();
		$logs[$clsISO->getUniqid()] = array(
			'title' => 'Hủy giao dịch',
			'content' => sprintf('<strong>%s</strong> đã hủy giao dịch này', $clsProfile->getFullName($profile_id, $oneProfile)),
			'reg_date' => time(),
		...
		$html = '<div class="gbox tr_'.$p_field.'_'.$toId.' tr_'.$p_field.'_'.$uid.' mb-2">
			<div class="gbox-body">
				<div class="form-group mb-2">
					...
					<input type="text" placeholder="Nhập tên..." name="'.$p_field.'['.$uid.'][title]" class="form-control required" maxlength="255" value="Tiêu chuẩn" />
				</div>			
				<div class="form-group mb-2">
				...
				<div class="form-group">
					<button type="button" onClick="$Core.helper.delete_sp_file(this, event)" uid="'.$uid.'" toId="'.$stock_id.'" class="btn btn-outline-default">'.$clsISO->makeIcon('bx-trash-alt', 'Xóa dòng').'</button>
				</div>
			</div>
		</div>';
	} else {
...
function default_pop_save_customer(){
	...
	if(!empty($phone) && $clsCustomer->countItem("phone='{$phone}' and admin_id='{$admin_id}'")){
		...
		if($clsCustomer->insert(array_merge($more, array(
			...
			if($auto_create_followups == 1){
				...
				foreach($list_times as $key => $date_id){
					...
					if($clsFollowUp->insert(array(
						...
						$titleNoty = sprintf('Lịch hẹn <strong>%s</strong> với khách hàng <strong>%s</strong> vào 
						lúc <strong>%s</strong>', $clsProperty->getTitle(_FOLLOWUP_CALL_ID) .": Call liên hệ lại khách", $clsCustomer->getName($customer_id), $clsISO->convertTimeToText($date_id, true));
			...
			if($admin_id != $profile_id){
				$titleNoty = sprintf('<strong>%s</strong> đã giao bạn phụ trách khách hàng <strong>%s</strong>', $clsProfile->getFullName($profile_id, $oneProfile), $clsCustomer->getName($customer_id, $oCustomer));
			...
			if(!empty($list_user_notify)){
				$titleNoty = sprintf('<strong>%s</strong> giao bạn liên quan tới khách hàng <strong>%s</strong>', $clsProfile->getFullName($profile_id, $oneProfile), $clsCustomer->getName($customer_id, $oCustomer));
...
function default_news(){
	...
	$field.= ",cat_id,events_config,start_date,end_date,regis_info";
	...
	if($news_id == 0){
		...
		if($is_staff_regis == 1 && $is_cusotmer_regis == 0){
			...
			if($clsNews->updateOne($news_id, array(
				...
				$msg = '_success';
				$html = '<a href="javascript:void(0);" onClick="cancel_register(this, event)" news_id="'.$news_id.'" class="awe__post-action awe__post-cancel-action text-danger" title="Hủy đăng ký" data-bs-toggle="tooltip">
					<i class="bx bxs-trash"></i> Hủy đăng ký
				</a>';
			}
...
function default_add_customer_line(){
	...
	$html = '<div class="widget-block '.$uid.' mb-3">
				<div class="widget-header d-flex align-items-center justify-content-between">
					...
					<button type="button" title="Xóa" data-bs-toggle="tooltip" regis_id="'.$regis_id.'" class="btn btn-primary p-1"><i class="bx bx-trash fs-11"></i></button>
				</div>
...
//	 $clsISO->print_pre($list_post); die();
	...
	$smarty->assign('total_record', $total_record);
	/*=============Title & Description Page==================*/
	$title_page = 'Bản tin tri �n Future Homes';
	$assign_list["title_page"] = $title_page;
	$description_page = $title_page;
	$assign_list["description_page"] = $description_page;
	$keyword_page = $title_page;
...
function default_pop_save_gratitude(){
	...
	if(!empty($department_id)) { // co chon phong ban
		foreach ($department_id as $department) {
			...
			if($department != _USER_DIRECTOR_ID) { // neu khong phai la phong B�H thi lay danh s�ch diem cong cua nhan vien phong do
				foreach($lst_profile as $key => $value) {
	...
	if($total_score_minus > $total_Lpoint && !$clsISO->checkSupper()) { // neu khong du diem va khong phai B�H
		$data = [
			"result"		=>	false,
