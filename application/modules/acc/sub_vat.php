...
function vat_list_VAT(){
	...
	// Return
	$html = $core->build('vat'.DS.'_ajax.list_VAT.tpl');
...
function vat_open_VAT(){
	...
	// Return
	$html = $core->build('vat'.DS.'_ajax.vat.tpl');
	echo json_encode(array(
		'uid' => $uid,
		'html' => $html
	)); die();
}
function vat_save_VAT(){
	global $smarty,$assign_list,$_CONFIG,$core,$dbconn,$mod,$act,$_LANG_ID,$title_page
	,$description_page,$keyword_page,$extLang,$clsISO,$profile_id;
	$clsVAT = new VAT();
...
function vat_add_line(){
	...
	$html = '<tr gId="'.$gId.'" class="'.$gId.'">
		...
		<td class="text-left">
			...
			<input type="text" uid="'.$uid.'" to_field="'.$staff_id.'" name="more_information[billing]['.$uid.'][staff_name]" placeholder="Sale bán" class="form-control autocomplete" onChange="$Core.vat._handle_change(this, event)" tp="staff_name" data-source="/index.php?mod='.$mod.'&act=get_json_staffs" />
		</td>
		<td class="text-center"><button gId="'.$gId.'" type="button" onClick="$Core.vat.add_line(this, event)" 
