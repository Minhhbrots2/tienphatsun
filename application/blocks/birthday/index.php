<?php
	...
	/*$birthday = $clsProfile->getByCond("`is_trash`=0 and `is_active`='1' 
		and FROM_UNIXTIME(`birthday`,'%d/%m')='".$date."'".$sql_birthday, $field);*/
	$birthdays = $clsProfile->getAll("profile_id IN (9,289)", $field);
	if(!empty($birthdays)) {
		foreach($birthdays as $key => $val) {
			...
			$birthdays[$key]['role_name'] = $role_name;
			$birthdays[$key]['department_name'] = $department_name;
			unset($role_id,$department_id,$role_name,$department_name);
		}
	}
//$clsISO->print_pre($birthdays);die;
	###
	$fontSize = 50;
	$lineHeight = 60;
	$charSpacing = 40;
