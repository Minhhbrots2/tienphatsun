	...
	*/
	...
	protected function _max_min_avg_sum($select = '', $alias = '', $type = 'MAX'){
		...
		if ( ! in_array($type, array('MAX', 'MIN', 'AVG', 'SUM'))){
			Common::show_error('Invalid function type: '.$type);
		}
	...
	protected function _where_in($key = NULL, $values = NULL, $not = FALSE, $type = 'AND '){
		...
		if($this->ar_where_group_started){
			$where_in = $this->_protect_identifiers($key) . $not . " IN (" . implode(", ", $this->ar_wherein) . ") ";
			$this->ar_where_group_started = FALSE;
		}else{
	...
	|-------------------------------
	|*/
	function _insert($table = '', $fields = NULL, $values=NULL){
		global $dbconn;
		$sql = "INSERT INTO ".$table." (".implode(', ', $fields).") VALUES (".implode(', ', $values).")";
		return !$dbconn->Execute($sql) ? 0:1;
	}
	/*
	|-------------------------------
	| Replace
	...
	*/
	public function delete($table = '', $where = '', $limit = NULL, $reset_data = TRUE){
		...
		if ($table == ''){
			if ( ! isset($this->ar_from[0])){
				if ($this->db_debug){
					trigger_error("Database nust set table", E_USER_ERROR);
				}
				return FALSE;
			}
			$table = $this->ar_from[0];
		...
		if (count($this->ar_where) == 0 && count($this->ar_wherein) == 0 && count($this->ar_like) == 0){
			if ($this->db_debug){
				trigger_error("Database nust use where", E_USER_ERROR);
			}
			return FALSE;
		}
		$kq = $this->_delete($table, $this->ar_where, $this->ar_like, $this->ar_limit);
		if ($reset_data){
			$this->_reset_write();
		}
		return $kq;
