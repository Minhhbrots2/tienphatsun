 ...
 */
class Mongo_DbException extends \CoreException {};
...
class Mongo_Db
{
	...
	public static function instance($name = 'default'){
		...
		if (empty(static::$instances)){
			if(Config::load('mongodb', TRUE, TRUE) ){
	...
	public function __construct(array $config = array()){
		...
		try{
			$this->connection = new MongoDB\Driver\Manager(trim($connection_string), $options);
			$this->db = new MongoDB\Database($this->connection, $config['database']);
			return $this;
	...
	public function get_cursor($collection = ""){
		...
		$a = new MongoDB\Collection($this->connection, $this->config['database'], $collection);
		
		
		$documents = $a->find($this->wheres, $this->selects)->limit((int) $this->limit)->skip((int) $this->offset)->sort($this->sorts);
