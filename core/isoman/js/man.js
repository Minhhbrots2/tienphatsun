$(document).ready(function(){
	...
	$(document).on('click', '.isoman-menu-cut,.isoman-menu-copy', function(ev){
		...
		$.cookie("isoman-action-dir",isoman_selected_files_dir());
	...
	$(document).on('click', '.isoman-menu-remove', function(ev){
		...
		var $_form = $_this.closest(".frmPop");
		var $isoman_for_id = $_form.find(".isoman-current-for_id").val();
		var $file_images = isoman_selected_files_dir();
		$(".isoman-menu").hide();
		var adata = {
		...
		if($("#isoman-current-typelist-"+$isoman_for_id).val()==1){
			...
			if($_this.hasClass("isoman-checked")){
				if(!$isoman_multiple){
					...
					$_this.removeClass("isoman-checked");
					if(isoman_selected_files_count()==0){
						$("#isoman-action").hide();
						$("#isoman-insert-"+$isoman_for_id).attr("isoman_url","").hide();
					}
				}
...
function isoman_selected_files_count(){
	var $str = '';
	var $i = 0;
...
function isoman_selected_files_dir(){
	var $str = '';
	$(".isoman-image").each(function(idx){
		var $_this = $(this);
		if($_this.hasClass('isoman-checked')){
			$str += $_this.attr('isoman_dir'); 
			$str += (idx==0)?'':'|';
		}
	});
	return $str;
