...
if( file_exists(dirname(__FILE__) . '/vietisothumb-config.php'))	require_once('vietisothumb-config.php');
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.vietisothumb.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'vietisothumb');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './tmp');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if(! defined('DEFAULT_Q') )				define ('DEFAULT_Q', 90);									// Default image quality. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_ZC') )			define ('DEFAULT_ZC', 0);									// Default zoom/crop setting. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_F') )				define ('DEFAULT_F', '');									// Default image filters. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_S') )				define ('DEFAULT_S', 0);									// Default sharpen value. Allows overrid in vietisothumb-config.php
if(! defined('DEFAULT_CC') )			define ('DEFAULT_CC', 'FFFFFF');							// Default canvas colour. Allows overrid in vietisothumb-config.php
...
/*
	...
	10. http://yoursite.com/path/to/vietisothumb.php?src=http://markmaunder.com/&webshot=1

	Notes on performance: 
	The first time a webshot loads, it will take a few seconds.
	From then on it uses the regular vietisothumb caching mechanism with the configurable options above
...
class vietisothumb {
	...
	protected $salt = "";
	protected $fileCacheVersion = 1; //Generally if vietisothumb.php is modifed (upgraded) then the salt changes and all cache files are recreated. This is a backup mechanism to force regen.
	...
	public static function start(){
		$vietiso = new vietisothumb();
		$vietiso->handleErrors();
		$vietiso->securityChecks();
		if($vietiso->tryBrowserCache()){
			exit(0);
		}
		$vietiso->handleErrors();
		if(FILE_CACHE_ENABLED && $vietiso->tryServerCache()){
			exit(0);
		}
		$vietiso->handleErrors();
		$vietiso->run();
	...
	public function __construct(){
		...
		if($this->isURL){
			if(ALLOW_ALL_EXTERNAL_SITES){
				...
				if(! $allowed){
					return $this->error("You may not fetch images from that site. To enable this site in vietisothumb, you can either add it to \$ALLOWED_SITES and set ALLOW_EXTERNAL=true. Or you can set ALLOW_ALL_EXTERNAL_SITES=true, depending on your security needs.");
				}
			}
		}
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<h1>A VietISOThumb error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />Query String : ' . htmlentities ($_SERVER['QUERY_STRING']);
		echo '<br />VietISOThumb version : ' . VERSION . '</pre>';
	}
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/vietisothumb_cacheLastCleanTime.touch';
		
		//If this is a new vietisothumb installation we need to create the file
		...
		if(@filemtime($lastCleanFile) < (time() - FILE_CACHE_TIME_BETWEEN_CLEANS) ){ //Cache was last cleaned more than 1 day ago
			...
			if ($files) {
				$vietisoeAgo = time() - FILE_CACHE_MAX_FILE_AGE;
				foreach($files as $file){
					if(@filemtime($file) < $vietisoeAgo){
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'vietisothumb_tmpimg_');
		...
		if($imgType == 'png' && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)){
			...
			$exec = PNGCRUSH_PATH;
			$tempfile2 = tempnam($this->cacheDirectory, 'vietisothumb_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'vietisothumb_tmpimg_');
	...
	protected function getLocalImagePath($src){
		...
		if(! $this->docRoot){
			...
			return $this->error("Could not find your website document root and the file specified doesn't exist in vietisothumbs directory. We don't support serving files outside vietisothumb's directory without a document root for security reasons.");
	...
	protected function serveWebshot(){
		...
		$instr = "Please follow the instructions at http://code.google.com/p/vietisothumb/ to set your server up for taking website screenshots.";
		...
		$vietisoeout = WEBSHOT_TIMEOUT * 1000;
		$ua = WEBSHOT_USER_AGENT;
		$jsOn = WEBSHOT_JAVASCRIPT_ON ? 'on' : 'off';
		$javaOn = WEBSHOT_JAVA_ON ? 'on' : 'off';
		$pluginsOn = WEBSHOT_PLUGINS_ON ? 'on' : 'off';
		$proxy = WEBSHOT_PROXY ? ' --http-proxy=' . WEBSHOT_PROXY : '';
		$tempfile = tempnam($this->cacheDirectory, 'vietisothumb_webshot');
		$url = $this->src;
		...
		if(WEBSHOT_XVFB_RUNNING){
			...
			$command = "$cuty $proxy --max-wait=$vietisoeout --user-agent=\"$ua\" --javascript=$jsOn --java=$javaOn --plugins=$pluginsOn --js-can-open-windows=off --url=\"$url\" --out-format=$format --out=$tempfile";
		} else {
			$command = "$xv --server-args=\"-screen 0, {$screenX}x{$screenY}x{$colDepth}\" $cuty $proxy --max-wait=$vietisoeout --user-agent=\"$ua\" --javascript=$jsOn --java=$javaOn --plugins=$pluginsOn --js-can-open-windows=off --url=\"$url\" --out-format=$format --out=$tempfile";
		}
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'vietisothumb');
	...
	protected function serveCacheFile(){
		...
		if(! is_file($this->cachefile)){
			$this->error("serveCacheFile called in vietisothumb but we couldn't find the cached file.");
			return false;
		}
	...
	protected function debug($level, $msg){
		if(DEBUG_ON && $level <= DEBUG_LEVEL){
			...
			error_log("VietISOThumb Debug line " . __LINE__ . " [$execTime : $tick]: $msg");
		}
	}
	protected function sanityFail($msg){
		return $this->error("There is a problem in the vietisothumb code. Message: Please report this error at <a href='http://code.google.com/p/vietisothumb/issues/list'>vietisothumb's bug tracking page</a>: $msg");
	}
	...
	protected function setMemoryLimit(){
		...
		$inibytes = vietisothumb::returnBytes($inimem);
		$ourbytes = vietisothumb::returnBytes(MEMORY_LIMIT);
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
			...
			curl_setopt ($curl, CURLOPT_WRITEFUNCTION, 'vietisothumb::curlWrite');
