...
error_reporting(E_ALL | ~ E_STRICT | ~ E_WARNING | ~E_ERROR | ~ E_NOTICE);
...
<head>
	...
	<link href="css/bootstrap-lightbox.min.css?v=<?php echo time(); ?>" rel="stylesheet" type="text/css" />
	<link href="css/style.css?v=<?php echo time(); ?>" rel="stylesheet" type="text/css" />
	<link href="css/dropzone.css?v=<?php echo time(); ?>" type="text/css" rel="stylesheet" />
	<!--[if lt IE 8]>
    <style>
        .img-container span {
...
<body>
    ...
    <div class="uploader">    
        ...
        <center>
            <button class="btn btn-large btn-inverse close-uploader"><i class="icon-backward icon-white"></i> <?php echo lang_Return_Files_List?></button>
        </center>
    ...
    <div class="container-fluid">
        ...
        <div class="row-fluid">
            ...
            <ul class="breadcrumb">
                ...
                ?>
                <li class="pull-right">
                	<a id="refresh" href="dialog.php?type=<?php echo $_GET['type']?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&subfolder=<?php echo $subfolder ?>&popup=<?php echo $popup;?>&field_id=<?php echo $_GET['field_id'] ? $_GET['field_id'] : '';?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $subdir ?>&<?php echo uniqid() ?>"><i class="icon-refresh"></i></a>
                </li>
            </ul>
        </div>
    	<!----- breadcrumb div end ------->
        ...
        <div class="row-fluid ff-container">
            ...
            <?php
                ...
                elseif(isset($_GET['type']) && $_GET['type']==0 && $_GET['field_id']==''){
                    $apply = 'apply_none';
                }
                elseif(isset($_GET['type']) && ($_GET['type']==3 || $_GET['type']==4 || $_GET['type']==5)){
                     $apply = 'apply_video';
                }
                else{
                    $apply = 'apply';
                ...
                foreach ($files as $file) {
                    if (is_dir($root . $cur_dir . $file) && ($file != '.' && !($file == '..' && $subdir=='')) ) {
                        ...
                        <li>
                            <figure>
                                <a title="<?php echo lang_Open ?>" href="dialog.php?type=<?php echo $_GET['type']?>&subfolder=<?php echo $subfolder ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&popup=<?php echo $popup;?>&field_id=<?php echo ((isset($_GET['field_id']) && !empty($_GET['field_id'])) ? $_GET['field_id'] : '');?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $src ?>&<?php echo uniqid() ?>">
                                    <?php if($file==".."){ ?>
                            ...
                            <div class="box"> <h4><?php echo $file ?></h4></div>
                            <figcaption>
                                <a href="javascript:void(0);" class="erase-button" <?php if($delete_folder){ ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo $root. $cur_dir .$file; ?>','<?php echo $thumbs_path.$subdir .$file; ?>'); $(this).parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>"> <i class="icon-trash <?php if(!$delete_folder) echo 'icon-white'; ?>"></i></a>
                            </figcaption>
                            <?php } ?>
                            </figure>
                        </li>
            	...
            	if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
            		...
            		if(in_array($file_ext, $ext)){
						if(in_array($file_ext, $ext_img)){
							...
							//echo $src_thumb; die();
							//add in thumbs folder if not exist 
							$thumb_path=dirname( __FILE__ ).DIRECTORY_SEPARATOR."thumbs".DS.$subfolder.DS.$subdir.$file;
							if(!file_exists($thumb_path)){
								 create_img_gd(dirname( __FILE__ ). DS.$current_path.$subfolder.DS.$subdir.$file, $thumb_path, 122, 91);
							}
                
                			$is_img=true;
            				//check if is smaller tha thumb
							$info = getimagesize(dirname( __FILE__ ). DS.$current_path.$subfolder.DS.$subdir.$file);
							if($info[0]<122 && $info[2]<91){
								$src_thumb=$current_path.$subfolder.DS.$subdir.$file;
								$show_original=true;
							}
            				...
            				if((!(isset($_GET['type']) && $_GET['type']==1 && !$is_img) && !(isset($_GET['type']) && $_GET['type']>=3 && !$is_video))){ ?>
