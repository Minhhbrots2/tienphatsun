...
function default_default(){
	global $assign_list,$_CONFIG,$_SITE_ROOT,$mod,$_LANG_ID,$act,$menu_current,$current_page,$oneSetting
		,$clsConfiguration,$core,$clsModule,$clsButtonNav,$oneSetting,$clsISO,$dbconn;
	...
	foreach($list_projects as $key => $val) {
		$arr_project_cached[$val["project_id"]] = $val;
	}
	...
	if(isset($_POST['filter'])&&$_POST['filter']=='filter'){
		...
		if(!empty($project_id)) $link .= '&project_id='.$project_id;
		if(!empty($block_id)) $link .= '&block_id='.$block_id;
		if(!empty($building_id)) $link .= '&building_id='.$building_id;
	...
	if(!empty($project_id) && !empty($block_id) && !empty($building_id)){
		$cond .= " and scope_slash like '%|".sprintf("%s!_%s!_%s",$project_id,$block_id,$building_id)."|%' escape '!'";
	}else if(!empty($project_id) && !empty($block_id) && empty($building_id)) {
		$prefix_block = sprintf("%s!_%s",$project_id,$block_id);
		$cond .= " and (scope_slash like '%|".$prefix_block."|%' escape '!' or scope_slash like '%|".$prefix_block."!_%' escape '!')";
	}else if(!empty($project_id) && empty($block_id) && empty($building_id)) {
		$json_project = sprintf('"project_id":"%s"', $project_id);
		$cond .= " and (scope_slash like '%|".sprintf("%s!_",$project_id)."%' escape '!' or scope like '%".$json_project."%')";
	}
	if(!empty($project_id)){		
	...
	if(!empty($allItem)){
		...
		foreach($allItem as $key => $val){
			...
			$building_name = [];
			if(!empty($val["scope"])) {
				$scope = $clsISO->to_array_json($val["scope"]);
				foreach($scope as $k_scope => $v_scope) {
					if(!empty($v_scope["project_id"])) {
						$project_name = $arr_project_cached[$v_scope["project_id"]]["title"];
					}
					if(!empty($v_scope["block_id"])) {
						$block_name = $arr_block_cached[$v_scope["block_id"]]["title"];
					}
					if(!empty($v_scope["building_id"])) {
						foreach($v_scope["building_id"] as $k => $v) {
							if(!empty($arr_building_cached[$v])) $building_name[] = $arr_building_cached[$v]["title"];							
						}
						
					}
				}
			}
			$allItem[$key]["project_name"] = $project_name;
			$allItem[$key]["block_name"] = $block_name;
			$allItem[$key]["building_name"] = implode(", ",$building_name);
	...
	if(!empty($scope)){
		...
		foreach($scope as $key => $val){
			...
			if($block_type==_BLOCK_TYPE_HIGHLEVEL_SALE){
				...
				if(empty($block_ids) && $project_id > 0){

					$all_blocks = $clsProperty->getAll("`property_type`='_BLOCK' and `for_id`='{$project_id}'", $clsProperty->pkey);

					if(!empty($all_blocks)){

						foreach($all_blocks as $one_block){

							$block_ids[] = $one_block[$clsProperty->pkey];
				...
				if(!empty($block_ids)){
					...
					#- Dự án chưa khai báo phân khu nào: vẫn ghi token cấp dự án để còn lọc được

					$scope_slash.= sprintf('|%s_0|', $project_id);
