...
function lowfloor_open_agency(){
	...
	$smarty->assign('lst_block',_PROJECT_BLOCK_CRAWL);
...
function lowfloor_save_agency(){
	...
	if(!empty($agency_id)) {
		...
		if($clsProperty->updateOne($agency_id, array(
			...
			$res = array("result" =>	true,"msg"	=>	"_success");
		}
	}
	// Return
	echo json_encode($res); die();
}
function lowfloor_open_sheet(){
	...
	// $clsISO->print_pre($arr_worksheets); die();
...
function lowfloor_handle_status(){
	global $smarty, $assign_list, $adminid, $core, $clsISO, $_LANG_ID;
	$clsProperty = new Property();
	$smarty->assign('clsProperty',$clsProperty);
	###
...
function lowfloor_crawl_agency(){
//	ini_set('display_errors', '1');
//ini_set('display_startup_errors', '1');
//error_reporting(E_ALL);
...
function lowfloor_open_config_column(){
	...
	if(!empty($agency_id) && !empty($ranges) && !empty($spreadsheetId) && !empty($target_id)) {
		...
		$highestColumnIndex = ($stock_type == _BLOCK_TYPE_HIGHLEVEL_SALE) ? 25 : 29;
...
function lowfloor_do_config_column(){
	require_once (DIR_INCLUDES.'/json_master/autoload.php');				
	require_once (DIR_INCLUDES . '/googleapiclient/vendor/autoload.php');
