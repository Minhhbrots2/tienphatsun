<div class="modal-dialog modal-md">
	<form class="modal-content" method="POST">
		<div class="modal-header"> 
			...
			<h3 class="modal-title"><strong>Phân quyền {if $source eq 'MF'}MF (myfuture.vn){else}MOC (myoceancity){/if}</strong></h3>
		</div>
			...
			{if !empty($list_permiss)}
				...
				<fieldset>
					...
					<div class="row">
						...
						<div class="col-12 col-md-3 mb-2">
							<div class="d-flex align-items-center" style="height: 40px">
								...
								<span>{$_oItem.title}{if $_oItem.is_default eq 1} <small class="text-muted" title="Mặc định của gói">(gói)</small>{/if}</span>
							</div>
						</div>
						{/foreach}
					</div>
					{if $_oGroup.code eq 'group_billing'}
						...
						<fieldset>
							<legend>Phân quyền Adminxx</legend>
		...
		<div class="modal-footer">
			<button type="button" class="btn btn-success" profile_id="{$profile_id}" profile_type="{$profile_type}" data-source="{$source}"
