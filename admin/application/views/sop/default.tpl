	...
	<div class="ui-title-bar">
		<div class="ui-title-bar__main-group">
			<div class="ui-title-bar__heading-group">
				<h1 class="ui-title-bar__title">Bảng hàng chuyển nhượng</h1>
			</div>
		</div>
			...
			<div class="next-tab__container">
				<ul class="next-tab__list filter-tab-list">
					<li class="filter-tab-item" data-tab-index="1">
						<a href="{$PCMS_URL}/index.php?mod={$mod}" class="filter-tab filter-tab-active show-all-items next-tab next-tab--is-active">Bảng hàng chuyển nhượng</a>
					</li>
				</ul>
			</div>
				...
				<form method="post">
					<div class="form-search d-flex justify-content-between align-items-center">
						<div class="form-inline d-flex align-items-end">
							...
							<div class="form-group mr-2">
								<label class="col-form-label mr-2">Chuyển nhượng HOT</label>
								<select name="is_hot" class="form-control iso-select2">
									<option value="">Chọn</option>
									<option value="1" {if $is_hot eq "1"}selected{/if}>Chuyển nhượng HOT</option>
									<option value="0" {if $is_hot eq "0"}selected{/if}>Không là chuyển nhượng HOT</option>
								</select>
							</div>
						...
						<div class="btn-group d-flex">
							<button type="button" class="btn btn-primary do_action d-none" data-action="approve_multi" onClick="$Core.sop.approve_sop(this, event);"><i class="fa fa-check" aria-hidden="true"></i> Duyệt</button>
							<button type="button" class="btn btn-danger do_action d-none" data-action="noapprove_multi" onClick="$Core.sop.show_notes(this,event)"><i class="fa fa-times" aria-hidden="true"></i> Không duyệt</button>
						</div>
					</div>
					<div class="hastable">
						<div class="freeze-table dragscroll">
							<table id="tableCall" cellspacing="0" class="table table-vertical table-striped" cellpadding="0" width="100%">
								...
								<tr{if $allItem[i].status_id eq $smarty.const._STATUS_STAFF_OFF_ID} class="tr_Off"{/if}>
									<td width="40px">
										<div class="checkbox">
											<input type="checkbox" class="checkitem sop_item" value="{$allItem[i].sop_id}" />
											<label></label>
										</div>
									</td>
									<td>{$smarty.section.i.iteration}</td> 
									<td class="border-end"><a class="fw-bold" href="{$clsClassTable->getLink($allItem[i].sop_id,$more_information.stock_code)}" target="_blank" stock_id="{$allItem[i].stock_id}" sop_id="{$allItem[i].sop_id}">{$allItem[i].stock_code}{if $more_information.having_dq eq 1} <span class="bg-danger text-danger px-2 rounded-2">ĐQ</span>{/if}</a></td>
									...
									<td>{if $allItem[i].is_locked eq 0 && $allItem[i].is_solded eq 0}
											Đang mở bán
										{else}
											{if $allItem[i].is_solded eq 1}Đã bán 
											{else}Đã khoá{/if}
										{/if}
									</td>
									<td class="text-center bg-gray">
										<a href="javascript:void(0);" class="SiteClickPublic" clsTable="Sop" pkey="{$pkeyTable}" 
									...
									<td class="text-center">
										<label class="switch">
										  <input type="checkbox" onChange="$Core.sop.verified(this, event)" 
											sop_id="{$allItem[i].sop_id}"{if $allItem[i].is_verified eq '1'} checked="checked"{/if} value="1" />
									...
									<td class="text-center" style="white-space: nowrap;">
										<div class="btn-group{if $smarty.section.i.last} dropup{/if}">
											...
											<ul class="dropdown-menu" style="right:0;left: unset;min-width: 100px;">
												<li {if $allItem[i].is_online eq 1} style="display: none"{/if}><a title="Duyệt" data-action="approve" href="javascript:void(0)" data-sop_id="{$allItem[i].sop_id}" onClick="$Core.sop.approve_sop(this,event)">
													{$core->makeIcon('check', 'Duyệt')}</a>
												</li>
												<li {if $allItem[i].is_online eq 2} style="display: none"{/if}><a title="Huỷ duyệt" data-action="noapprove" data-sop_id="{$allItem[i].sop_id}" href="javascript:void(0)" onClick="$Core.sop.show_notes(this,event)">
													{$core->makeIcon('close', 'Không duyệt')}</a>
												</li>
												<li {if $allItem[i].is_online eq 0 || $allItem[i].is_online eq 2} style="display: none"{/if}><a title="Huỷ duyệt" data-action="unapprove" data-sop_id="{$allItem[i].sop_id}" href="javascript:void(0)" onClick="$Core.sop.approve_sop(this,event)">
													{$core->makeIcon('refresh', 'Huỷ duyệt')}</a>
												</li>
												<li><a title="Xoá" data-action="unapprove" data-sop_id="{$allItem[i].sop_id}" href="javascript:void(0)" onClick="delete_sop(this,event)"> 
...
<script type="text/javascript" src="{$URL_VIEWS}/sop/js/jquery.sop.js?v={$upd_version}"></script>
