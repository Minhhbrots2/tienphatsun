				...
				<div class="form-group">
					<div class="col-xs-12 col-md-12">
                        ...
                        <div class="input-group">    
                            <input id="map_address" name="map_address" value="{$more_information.map_address}" type="text" class="form-control" />
                            <div class="input-group-btn">
                                <button type="button" id="search_map" class="btn btn-default">{$core->makeIcon('search', 'Tìm')}</button>
                            </div>
                        </div>
	...
	<div class="ui-card__section">
		<header class="ui-stack ui-stack--wrap mb-3">
			...
			<div class="ui-stack-item">
				<button type="button" class="btn btn-default btn-add_property-map" _openFrom="_project" project_id="">+ Thêm</button>
			</div>
		</header>
		<div class="ui-type-container">
			<div class="p-md-3 box-location-highlight box-group-location">
				...
				<div class="form-group">
					<div class="col-md-12">
						<label class="col-form-label">Nhấc con trỏ đặt vào địa điểm của bạn.</label>
						 <div style="width:100%; height:400px; overflow:hidden;z-index:1" id="my_canvas_highligh"></div>
					</div>
				</div>
				...
				<div class="form-group item-location mt-3 d-flex align-item-center">
					<div class="form-row">
...
<script type="text/javascript">
    // if (user_id == '64') {
        $(function(){	
            var mapLeaflet, mapLeafletLocation, mapLeafletHighlight;
            ...
            function initializeMapHighlight() {
                ...
                if(!$('#my_canvas_highligh').hasClass('initialize')) {
                    $('#my_canvas_highligh').addClass('initialize')
                    mapLeafletHighlight = intiMapLeaflet('my_canvas_highligh');
                ...
                if (locationItem.length > 0) {
                    locationItem.not('initialize').each(function(index) {
                        ...
                        let self = $(this);
                        let markerHighlight;
                        let mapAddressHighlight = $(this).find('.map_address_highlight');
                        ...
                        if ( typeof mapLa != 'undefined' && typeof mapLo != 'undefined' && mapLa.length > 0 && mapLo.length > 0) {
                            markerHighlight = L.marker([mapLa, mapLo], {
                                draggable: true
                            }).addTo(mapLeafletHighlight);
                            markerHighlight.on('dragend', function (e) {
                                const position = markerHighlight.getLatLng();
                                self.find('.input_map_highlight_lat').val(position.lat.toFixed(6));
                                self.find('.input_map_highlight_lng').val(position.lng.toFixed(6));
                            });
                        }
                        ...
                        suggestionsEl.on('click', 'li', function () {
                            ...
                            $(this).parents('.item-location').find('.map_address_highlight').val(name);
                            suggestionsEl.empty();
                            if (markerHighlight != null) {
                                markerHighlight.remove();
                            }
                            mapLeafletHighlight.setView([lat, lon], 16);
                            self.find('.input_map_highlight_lat').val(lat);
                            self.find('.input_map_highlight_lng').val(lon);
