	...
	<div class="ui-title-bar">
		<div class="ui-title-bar__main-group">
			<div class="ui-title-bar__heading-group">
				<h1 class="ui-title-bar__title w-100">Danh sách đại lý cập nhật</h1>
				...
				<div class="hastable table-wrapper">
					<table class="table table-bordered mb-0" cellspacing="0" cellpadding="0" width="100%">
						<thead><tr>
							...
							<th class="align-center text-center" width="20%" colspan="{$lst_block|@count}">Phân khu</th>
							<th class="align-center text-right align-center" width="60px" rowspan="2">{$core->get_Lang('Actions')}</th>
						</tr>
						<tr>
							{foreach from=$lst_block item=_block_name key=key}
							<th class="text-center" width="20%">{$_block_name}</th>
							{/foreach}
						</tr></thead>
						<tbody>
						{if !empty($lstAgency) }
							...
							{assign var = block_crawl value = $_oItem.block_crawl}
								...
								<tr class="tr_agency tr_agency_{$_oItem.property_id}">
									...
									<td class="align-center text-center">
										<button agency_id="{$_oItem.property_id}" type="button" class="btn btn-default" onClick="$Core.property.crawlAgency(this,event)">
									...
									{foreach from=$lst_block item=_block_name key=key}
										<td class="text-center">
											<label class="switch">
												<input type="checkbox" onchange="$Core.property.setStatusCrawl(this, event)" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}" project_id="" block_id="{$key}" agency_id="{$_oItem.property_id}" value="1" class="switch_{$clsISO->getUniqid()}" name="is_crawl" {if !empty($block_crawl[$key].is_crawl)}checked{/if}>
												<span class="slider round"></span>
											</label>
										</td>
									{/foreach}
									<td class="text-center" data-label="{$core->get_Lang('Actions')}">
										<button class="btn btn-icon btn-default" onClick="$Core.property.open_agency_crawl(this,event)" stock_type="{$smarty.const._BLOCK_TYPE_HIGHLEVEL_SALE}" class="btn btn-default" agency_id="{$_oItem.property_id}">{$core->makeIcon('pencil')}</button>
									</td>
								</tr>
								{/if}
							{/foreach}
