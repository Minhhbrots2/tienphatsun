<div class="ui-title-bar-container">
	...
	<div class="ui-title-bar__main-group">
		<div class="ui-title-bar__heading-group">
			<h1 class="ui-title-bar__title">{$core->get_Lang('Edit Customer')}
		</div>
	</div>
</div>
<div class="ui-layout">
					...
					<div class="box-body">
						<div class="form-group">
							<div class="row">
								<div class="col-md-6">
									<label>Họ</label>
									<input type="text" name="iso-address_first_name" class="form-control" value="{$oneItem.address_first_name}" placeholder="Nhập họ" />
								</div>
								<div class="col-md-6">
									<label>Tên</label>
									<input type="text" name="iso-address_last_name" class="form-control" value="{$oneItem.address_last_name}" placeholder="Nhập tên" />
								</div>
							</div>
			...
			<div class="col-md-4">
				<div class="box light">
					<div class="box-title no-bottom-border">
						<div class="caption"><span class="bold">Ghi chú</span></div>
					</div>
					<div class="box-body">
						<textarea class="form-control" name="note" placeholder="Nhập ghi chú" rows="5">{$oneItem.note}</textarea>
					</div>
                    ...
                    <div class="box-end no-border-top">
                    	<select class="form-control" size="3" id="profile_group" multiple="multiple" name="group[]">
							{foreach from=$lstGroup item = group}
                            <option {if $clsClassTable->checkInGroup($pvalTable,$group.property_id,$oneItem)}selected="selected"{/if} value="{$group.id}">{$clsProperty->getTitle($group.property_id)}</option>
                            {/foreach}
                        </select>
                    </div>
				</div>
...
<script type="text/javascript">
	...
	var district_id = '{$oneItem.district_id}';
</script>
{literal}
<script type="text/javascript">
	$(function(){
