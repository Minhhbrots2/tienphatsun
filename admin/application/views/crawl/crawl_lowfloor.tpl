							...
							{assign var = crawl_lowfloor value = $_oItem.crawl_lowfloor}
								<tr class="tr_agency tr_agency_{$_oItem.property_id}">
									<td class="text-center" data-label="{$core->get_Lang('Actions')}">
										<button class="btn btn-icon btn-default" onClick="$Core.crawl.open_agency(this,event)" stock_type="{$smarty.const._BLOCK_TYPE_LOWFLOOR_SALE}" class="btn btn-default" agency_id="{$_oItem.property_id}">{$core->makeIcon('pencil')}</button>
									</td>
									<td class="text-nowrap" data-label="Tiêu đề">{$_oItem.title}</td>
									{foreach from=$lst_project item=_project_name key=key}
										<td class="text-center">
											<label class="switch">
												<input type="checkbox" onchange="$Core.crawl.handle_status(this, event)" stock_type="{$smarty.const._BLOCK_TYPE_LOWFLOOR_SALE}" project_id="{$key}" block_id="" agency_id="{$_oItem.property_id}" value="1" class="switch_{$clsISO->getUniqid()}" name="is_crawl" {if !empty($crawl_lowfloor[$key].is_crawl)}checked{/if}>
