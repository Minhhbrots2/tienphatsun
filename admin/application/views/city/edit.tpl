<div class="breadcrumb">
	...
	<a>&raquo;</a>
    <a href="{$PCMS_URL}/index.php?mod={$mod}{$pUrl}" title="{$core->get_Lang('cities')}">{$core->get_Lang('cities')}</a>
    <a>&raquo;</a>
    <a href="{$curl}" title="{$act}"> {if $pvalTable}{$core->get_Lang('edit')} #{$pvalTable}{else}{$core->get_Lang('add')}{/if}</a>
    <!-- Back-->
    <a href="javascript:window.history.back();" class="back fr">{$core->get_Lang('back')}</a>
</div>
{if $msg eq 'DuplicateCity'}
...
<div class="container-fluid">
    <div class="page-title">
        <h2>{if $pvalTable}{$clsClassTable->getTitle($pvalTable)}{else}{$core->get_Lang('Add New Cities')}{/if}</h2>
    ...
    <div id="clienttabs">
        <ul>
            ...
            <li><a href="javascript:void();"><i class="iso-bassic"></i> {$core->get_Lang('introinformation')}</a></li>
    	...
    	<div id="tab_content">
            <div class="tabbox" style="display:block">
                <div class="photobox fl mr20 image">
					...
					{if $oneItem.image}
					<a pvalTable="{$pvalTable}" clsTable="City" href="javascript:void()" title="{$core->get_Lang('delete')}" class="photobox_edit deleteItemImage" g="imgItem">X</a>
					{/if}
                    {else}
                ...
                <div class="fl span75">
                    <div class="span100">
                        <div class="row-span" style="margin-bottom:10px; padding-bottom:10px;">
							 <div class="fieldlabel">{$core->get_Lang('City Name')} <span class="requiredMask">*</span> </strong></div>
							 <div class="fieldarea">
                    ...
                    <div class="row-span">
                        <div class="fieldlabel">{$core->get_Lang('continents')}*:</div>
                        <div class="fieldarea">
                            <select class="glSlBox required full" name="iso-continent_id" style="font-size:14px;font-weight:bold;padding:4px;">
                    ...
                    {/if}
                    {if $clsConfiguration->getValue('SiteModActive_country') and $core->checkAccess('country')}
                    <div class="row-span" id="boxCountry">
                        <div class="fieldlabel">{$core->get_Lang('country')} <font class="required">*</font></div>
                        <div class="fieldarea">
                            <select class="glSlBox required full" id="country_id" name="iso-country_id" style="width:200px">
                                {$clsCountry->makeSelectboxOption($country_id,$continent_id)}
                            </select>
                        </div>
                    </div>
                    {/if}
                    {if $core->checkAccess('area') && $clsConfiguration->getValue('SiteActive_region')}
                    <div class="row-span" id="boxVungMien" {if !$clsRegion->checkExits($oneItem.country_id)}style="display:none"{/if}>
                        <div class="fieldlabel">{$core->get_Lang('regions')} <font class="required">*</font></div>
                        <div class="fieldarea">
                            <select class="glSlBox full" id="slb_RegionID" name="iso-region_id" style="width:200px">
					...
					<script type="text/javascript">
						...
						function loadSelectRegion($country_id,$region_id){
                            ...
                            $.ajax({
                                type: "POST",
                                url:path_ajax_script+'/index.php?mod=city&act=ajSelectRegion',
                    ...
                    <div class="row-span">
                        <span class="notice bold" style="font-size:16px; padding-left:0;"><span class="requiredMask">*</span> {$core->get_Lang('Descriptions')} {$clsClassTable->getTitle($pvalTable)}</span>
                    ...
                    <div class="row-span">
                        ...
                        <div class="fieldarea-full" style="width:100%">
                            ...
                            <div style="width:100%; height:220px; overflow:hidden; position:relative">
                                <div id="map_canvas" style="width:100%; height:220px; overflow:hidden"></div>
            ...
            <div class="tabbox" style="display:none">
                <div class="row-span">
                    <span class="notice bold" style="font-size:16px; padding-left:0;">{$core->get_Lang('introplacevisit')}</span>
                    <div class="clearfix" style="height:5px"></div>
                    <div class="fieldarea" style="width:100%">{$clsForm->showInput('intro_attraction')}</div>
                </div>
                <div class="row-span">
                    <span class="notice bold" style="font-size:16px; padding-left:0;">{$core->get_Lang('introtips')}</span>
                    <div class="clearfix" style="height:5px"></div>
                    <div class="fieldarea" style="width:100%">{$clsForm->showInput('intro_travel')}</div>
                </div>
			</div>
            {if $pvalTable}
            <div class="tabbox" style="display:none">
            	<div class="row-field">
                    ...
                    <div class="coltrols">
                        <input class="text full" name="config_value_title" value="{$clsISO->getPageTitle($pvalTable,City)}" maxlength="255" type="text">
                ...
                <div class="row-field">
                    ...
                    <div class="coltrols">
                        <textarea name="config_value_intro" class="text full" style="height:60px">{$clsISO->getPageDescription($pvalTable,City)}</textarea>
                        <div class="clearfix mt5"></div>
                ...
                <div class="row-field">
                    ...
                    <div class="coltrols">
                        <textarea name="config_value_keyword" class="text full" style="height:60px">{$clsISO->getPageKeyword($pvalTable,City)}</textarea>
                        <div class="clearfix mt5"></div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('select[name=iso-continent_id]').change(function(){
			var _this=$(this);
			$('select[name=iso-country_id]').html('<option value="">'+loading+'</option>');
			$.ajax({
				type: "POST",
				url: path_ajax_script+'/?mod='+mod+'&act=makeSelectboxCountry',
				data: {"continent_id"	: _this.val()},
				dataType: "html",
				success: function(html){
					vietiso_loading(0);
					if(html.indexOf('_NOT_COUNTRY') >= 0) {
						$('#boxVungMien').hide();
						$('#boxCountry').hide();
					} else {
						$('#boxVungMien').show();
						$('#boxCountry').show();
						$('select[name=iso-country_id]').html(html);
						loadCity();
					}
				}
			});
