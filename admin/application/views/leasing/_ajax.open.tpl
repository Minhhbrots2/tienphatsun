<div class="modal_leasing modal-dialog modal-dialog-centered modal-ipad"> 
	<div class="modal-content">
		<div class="modal-header position-relative d-block">
			<div class="box_header_modal_leasing d-flex m-n1 align-items-center justify-content-between">
				<h5 class="modal-title fs-3 text-main">{$clsLeasing->getCode($oneLeasing)}</h5>
				<h4 class="text_price mb-0 text-main fw-bold fs-5">
					{if $oneLeasing.is_locked eq '1'}
						<i data-bs-toggle="tooltip" data-bs-trigger="hover" title="Đã khóa" class="material-icons-outlined fs-small text-danger">lock</i>
					{/if}
					
					{$clsISO->priceFormatV2($oneLeasing.price,4)} tỷ
				</h4>
			</div>
			<div class="d-flex align-items-center justify-content-between">
				<div class="d-flex align-items-center p_left">
					<span class="text-muted fs-12 mr-1">
						<img class="avatar avatar-xs" onerror="this.src='{$URL_IMAGES}/no-avatar.jpg'" src="{$clsProfile->getAvatar($oneLeasing.user_id , 30, 30)}" />
					</span>
					<span class="text-muted fs-12">
						<i class="fa fa-clock-o" aria-hidden="true"></i> {$clsISO->getTimeAgo($oneLeasing.upd_date)} 
					</span>
				</div>
				<div class="d-flex align-items-center p_right">
					<div class="approve verified d-inline-flex {if $oneLeasing.is_online eq 2} bg-red text-red{else if $oneLeasing.is_online eq 1} bg-green text-green{else} bg-yellow text-yellow{/if} fs-12 py-1 px-2 align-items-center gap-1 w-max rounded-pill mr-2">
						...
						{if $oneLeasing.is_online eq 2}
							Không kiểm duyệt
						{else if $oneLeasing.is_online eq 1}
							Đã kiểm duyệt
						{else} 
							Chờ kiểm duyệt
						{/if}
					</div>
					{if $oneLeasing.fee_included eq '1'}
				...
				<div class="rounded-2 mb-2 overflow-hidden">
					<div class="d-flex highlight tEzTuYtLau bg-grayter p-lg-2 mb-0">
						<div class=" text-center flex-fill">
							...
							<div class="txt_highlight fw-bold text-main">{$clsProperty->getTitle($oneLeasing.bedroom_id)}</div>
						</div>
						<div class=" text-center flex-fill"> 
							...
							<div class="txt_highlight fw-bold text-main">{$clsProperty->getTitleQR($oneLeasing.home_direction_id)}</div>
						</div>
						<div class=" text-center flex-fill">
					...
					<div id="{$clsISO->getUniqid()}" class="slideshow owl-carousel mb-0">
						...
						<div class="d-flex overflow-hidden slideshow-item align-items-center justify-content-center">
							<img src="{$domain_myocean}{$_oMedia.image}" alt="{$oneLeasing.stock_code}" />
						</div>
						{/foreach}
					</div>
					{/if}
				...
				<div class="w-100 d-flex flex-wrap ox-sm:col-4 align-items-center gap-2">
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							<li><a class="dropdown-item" href="{$PCMS_URL}/cn/edit/{$leasing_id}" class="btn flex-fill btn-outline-default">
							...
							<li><a class="dropdown-item text-muted" leasing_id="{$leasing_id}" href="javascript:void(0);">
								<i class="material-icons-outlined">storefront</i> Đã bán</a></li>
							{else}
							<li><a class="dropdown-item" onClick="$Core.leasing.mark_sold(this, event)" leasing_id="{$leasing_id}" href="javascript:void(0);">
					...
					{/if}
					<a href="tel:{$oneLeasing.contact_phone}" class="btn flex-fill btn-outline-default">
						<i class="fa fa-phone" aria-hidden="true"></i> {$oneLeasing.contact_phone}</a>
					<a href="https://zalo.me/{$oneLeasing.contact_phone}" class="btn flex-fill btn-outline-default">
						<img src="{$URL_IMAGES}/zalo_chat.png" width="20px" /> Chat Zalo</a>
					<a type="button" class="btn flex-fill btn-outline-default close js_close_pop_{$leasing_id}" data-dismiss="modal" >Đóng cửa sổ</a>
				</div>
			</div>
		</form>
	</form>
