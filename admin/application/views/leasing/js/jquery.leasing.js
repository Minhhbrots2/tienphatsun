...
$Core.leasing = {
	...
	approve_leasing: function (_this, e){
		e.preventDefault(); 
		var action = $(_this).data('action'),
			_total_checked = 0, leasing_ids = [], $_adata={}, notes='';
		$_adata['action'] = action;
		if($(_this).hasClass('disabled')){
			return false;
		}
		if(action == "approve_multi"){
			if($('#tableCall .leasing_item:checked').length){
				$('#tableCall .leasing_item:checked').each((_i, _elem) => { 
					_total_checked++;
					leasing_ids.push($(_elem).val()); 
				});			
			}
			$_adata['leasing_id'] = leasing_ids;
		}else if(action == "noapprove_multi" || action == "noapprove"){
			_total_checked = 1;
			var _form = $(_this).closest('form');
			leasing_ids = $("input[name='leasing_ids']", _form).val(); 
			notes = $("textarea[name=notes]", _form).val();
			$_adata['notes'] = notes;
			$_adata['leasing_id'] = leasing_ids;
		}else{
			_total_checked = 1;
			leasing_id = $(_this).data('leasing_id');
			$_adata['leasing_id'] = leasing_id;
		}
		if(_total_checked > 0){
			$.post(path_ajax_script+'/index.php?mod='+mod+'&act=approve_leasing', $_adata, function(html){
			...
			$Core.alert.error("Bạn chưa chọn danh sách bảng hàng cho thuê !!!");
		}
		return false;
	},
	show_notes: function (_this,e){
		e.preventDefault(); 
		var action = $(_this).data('action'),
			_total_checked = 0, leasing_ids = [];
		if($(_this).hasClass('disabled')) {
			return false;
		}
		if(action == "noapprove_multi"){
			if($('.leasing_item:checked').length){
				$('.leasing_item:checked').each((_i, _elem) => { 
					_total_checked++;
					leasing_ids.push($(_elem).val()); 
				});
			}
			...
			_total_checked = 1;
			leasing_ids = $(_this).data('leasing_id');
		}
		if(_total_checked > 0){
			$.post(path_ajax_script+'/index.php?mod='+mod+'&act=show_notes', { 
				'action':action,
				'leasing_id' : leasing_ids
			}, function(html){
				if(html != ''){
...
function delete_leasing(_this, e){
	e.preventDefault(); 
	var leasing_id = $(_this).data('leasing_id');
	if (confirm('Bạn có chắc chắn muốn xoá?')) {
		$.post(path_ajax_script+'/index.php?mod='+mod+'&act=delete_leasing', {
		'leasing_id' : leasing_id
	}, function(html){
		if(html.indexOf('_success') >= 0){
			alertify.success("Success !");
