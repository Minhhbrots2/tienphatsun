	...
	<div class="ui-title-bar">
		<div class="ui-title-bar__main-group">
			<div class="ui-title-bar__heading-group">
				<h1 class="ui-title-bar__title w-100">Khóa học</h1>
						...
						<form method="post">
							<div class="form-search form-inline">
								...
								<div class="form-group pull-right">
									...
									<a href="javascript:void(0)" class="btn btn-danger text-white btn-delete-all" style="display: none" clsTable="FAQ"> 
							...
							<div class="hastable table-wrapper">
								...
								<table cellspacing="0" class="table table-vertical table-striped" width="100%">
									<thead><tr>
										...
										<th class="text-center" colspan="4" style="width:4%">{$core->get_Lang('move')}</th>
									...
									<tr class="{if $smarty.section.i.index%2 eq 0}row1{else}row2{/if}">
										...
										<td class="text-left">
											<a href="{$PCMS_URL}/?mod={$mod}&act=edit&faq_id={$core->encryptID($allItem[i].faq_id)}">{$clsClassTable->getTitle($allItem[i].faq_id)}</a>
										...
										<td class="text-center">
											...
											<a title="{$core->get_Lang('movetop')}" href="{$PCMS_URL}/index.php?mod={$mod}&act=move&direct=movetop&faq_id={$core->encryptID($allItem[i].faq_id)}{$pUrl}"><i class="icon-circle-arrow-up"></i></a>
											{/if}
										</td>
										<td class="text-center">
											{if !$smarty.section.i.last}
											<a title="{$core->get_Lang('movebottom')}" href="{$PCMS_URL}/index.php?mod={$mod}&act=move&direct=movebottom&faq_id={$core->encryptID($allItem[i].faq_id)}{$pUrl}"><i class="icon-circle-arrow-down"></i></a>
											{/if}
										</td>
										<td class="text-center">
											{if !$smarty.section.i.first}
											<a title="{$core->get_Lang('moveup')}" href="{$PCMS_URL}/index.php?mod={$mod}&act=move&direct=moveup&faq_id={$core->encryptID($allItem[i].faq_id)}{$pUrl}"><i class="icon-arrow-up"></i></a>
											{/if}
										</td>
										<td class="text-center">
											{if !$smarty.section.i.last}
											<a title="{$core->get_Lang('movedown')}" href="{$PCMS_URL}/index.php?mod={$mod}&act=move&direct=movedown&faq_id={$core->encryptID($allItem[i].faq_id)}{$pUrl}"><i class="icon-arrow-down"></i></a>
											{/if}
										</td>
