						...
						<div class="ui-card__section">
							<div class="ui-type-container">
								...
								<div class="form-group">
									...
									<select name="iso-cat_id" class="form-control iso-selectize required">
										{$clsProperty->getSelectByProperty('_FAQs', $cat_id)}
									</select>
								</div>
								<div class="form-group row">
									<div class="col-md-6">
										<label class="col-form-label">{$core->get_Lang('Start Date')}</label>
										<input class="form-control w-100 timepicker" placeholder="dd/mm/yy" required name="start_date"{if $pvalTable gt '0'} value="{$clsISO->convertTimeToText($oneItem.start_date, true)}"{/if} />
									</div>
									<div class="col-md-6">
										<label class="col-form-label">{$core->get_Lang('Due Date')}</label>
										<input class="form-control timepicker" placeholder="dd/mm/yy" required name="due_date"{if $pvalTable gt '0'} value="{$clsISO->convertTimeToText($oneItem.due_date, true)}"{/if} />
									</div>
								</div>
								<div class="form-group">
									<label class="col-form-label">{$core->get_Lang('Location')}</label>
									<input type="text" class="form-control required" name="iso-location" value="{if $pvalTable gt '0'}{$oneItem.location}{/if}" required maxlength="255" placeholder="HA02-211 Vinhomes Ocean Park 1"{if $pvalTable gt '0'} value="{$oneItem.address}"{/if} />
								</div>
								<div class="form-group">
