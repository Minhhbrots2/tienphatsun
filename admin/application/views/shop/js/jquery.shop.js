...
$Core.shop = {
	open: function(_this, e){
		...
		if($(_this).hasClass("dev")) {
			$_adata["dev"] = 1;
		}
		
		$.post(path_ajax_script+'/index.php?mod='+mod+'&act=open', $_adata, function(html){
			vietiso_loading(0);
	...
	upload_file: function(_this, e){
		...
		form.ajaxSubmit({
			type : 'POST',
			url : path_ajax_script+'/index.php?mod=shop&act=upload_file',
			dataType:'json',
			success : function(respJson){
				vietiso_loading(0);
				form.clearForm();
    ...
    select_file2: function(_this, e){
		e.preventDefault();
		var toId = $(_this).attr('toId'),
			for_id = $(_this).attr('for_id'),
			folder_id = $(_this).attr('folder_id');
		$('#'+toId).attr({'folder_id': folder_id,'for_id':for_id}).trigger('click');
		return false;
	},	
	upload_image: function(_this, e){
		...
		form.ajaxSubmit({
			type : 'POST',
			url : path_ajax_script+'/index.php?mod=shop&act=upload_image',
			dataType:'json',
			data : $_adata,
			success : function(respJson){
				vietiso_loading(0);	
				...
				form.resetForm();			
				if(respJson.msg.indexOf('_success') >= 0){
					$Core.shop.genImage(respJson,for_id,toId);
				}else{
					alertify.error("Lỗi upload ảnh");
				}				
			}
		});	
		return false;
	},
	genImage: function (respJson,for_id,toId) {
		var arr_image_thumb = respJson.arr_image_thumb,
			arr_image_full = respJson.arr_image_full;
		let name = for_id;
		if (for_id == 'image-content') {
			name="image_gallery[]";
		}
		if (for_id == 'image-menu') {
			name="image_menu[]";
		}
		$.each(arr_image_thumb, function( index, value ) {
			let html = `<div class="item-img">
							<img width="100" height="100" id="isoman_show_`+for_id+`" src="`+value+`">
							<button type="button" class="btn btn-del-image" onClick="$Core.shop.del_photo(this, event)"><i class="fa fa-times-circle" aria-hidden="true"></i></button>
							<input type="hidden" name="`+name+`" value="`+arr_image_full[index]+`">
						</div>`;
			$(`#`+for_id+`_`+toId).find('.img-empty').before(html);
		});
	}
}